...
import sparkles.dql : DqlSchema, DqlEngine, DqlFilter, evalDql, parseDql,
    printDqlHelp;
import sparkles.input;
import sparkles.ui_sdl3;

/// The filtering schema over the shared input vocabulary, generated by the
/// reflection kernel — the example writes no paths of its own.
alias EventSchema = DqlSchema!Event;
...
struct InputEcho
{
    ...
    @(Option(`F|event-filter`,
        description: "DQL filter for events, or 'help' for the generated schema"))
    string eventFilter;
    ...
    Expected!(void, string) run()
    {
        ...
        {
            if (eventFilter.among!("?", "help"))
            {
                printDqlHelp!EventSchema("input-echo");
                return ok();
            }
            auto compiled = parseDql!EventSchema(engine, eventFilter);
            if (compiled.hasError)
                return err!void("invalid event filter: "
                    ~ compiled.error.message);
            filter = compiled.value.move;
            hasFilter = true;
        }
        ...
        for (int i = 0; !quit && (frames == 0 || i < frames); i++)
        {
            ...
            events.poll((Event e) {
                if (!hasFilter || evalDql!EventSchema(engine, filter, e))
                    writeln(describe(e));
                quit |= isEscape(e);
            }, cell, cell);
