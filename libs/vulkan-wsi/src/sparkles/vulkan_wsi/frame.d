...
unchanged.
*/
module sparkles.vulkan_wsi.frame;
...
struct FrameSync
{
    ...
    */
    static PresentationResult!void create(Context)(out FrameSync sync,
        ref Context vk,
    ...
    */
    VkExpected!void waitForFrame(Context)(ref Context vk,
        ulong timeout = ulong.max)
        @system nothrow
    {
    ...
    */
    VkExpected!void waitForImage(Context)(ref Context vk, uint imageIndex,
    ...
    */
    VkExpected!void beginFrame(Context)(ref Context vk, uint imageIndex)
        @system nothrow
    in (imageIndex < _imageGuard.length, "image index is out of range")
    {
        auto fence = _inFlight[_frame];
    ...
    */
    VkExpected!void submit(Context)(ref Context vk, VkCommandBuffer cmd,
        uint imageIndex)
        @system nothrow
    in (imageIndex < _renderFinished.length, "image index is out of range")
    {
    ...
    */
    VkExpected!void waitAll(Context)(ref Context vk,
        ulong timeout = ulong.max)
        @system nothrow
    {
    ...
    */
    PresentationResult!void replaceImages(Context)(ref Context vk,
        uint imageCount)
        @system nothrow
    in (imageCount > 0, "a swapchain always has at least one image")
    {
...
@("vulkan_wsi.frame.suboptimalFinishesTheFrameOutOfDateAbandonsIt")
...
@("vulkan_wsi.frame.timeoutIsNeitherAFrameNorAFailure")
@safe pure nothrow @nogc unittest
{
    with (VkResult)
    {
...
@("vulkan_wsi.frame.frameSlotsWrapIndependentlyOfImageCount")
...
@("vulkan_wsi.frame.waitAllOnEmptySyncIsANoOp")
...
@("vulkan_wsi.frame.forgetImagesClearsGuardsAndKeepsTheSlots")
@system nothrow unittest
{
    FrameSync sync;
    sync._inFlight = new VkFence[2];
...
@("vulkan_wsi.frame.hasRetiredTracksTheRetiredList")
...
@("vulkan_wsi.frame.destroyIsIdempotentOnAHalfBuiltSync")
@system nothrow unittest
{
