 ...
 * $(UL
 *   $(LI `writeTree` — writes to any output range (including @nogc `UniqueBuffer`))
 *   $(LI `drawTree` — convenience overload returning `string`)
 * )
 */
module sparkles.ui.components.tree_draw;

import sparkles.base.term_style : Style;
import sparkles.base.text.writers : writeStylized, writeValue;
...
@("treeDraw.traits.hasTreeChildren")
@safe pure nothrow @nogc
unittest
{
