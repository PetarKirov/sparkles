...
Size popupBound(in Palette pal, in Rect boundary) @safe pure nothrow @nogc
{
    ...
    return Size(
        fit(pal.overlayMaxWidth, boundary.width, pal.overlayMinWidth),
        fit(pal.overlayMaxHeight, boundary.height, pal.overlayMinHeight));
}
    ...
    in Size content, in Rect boundary) @safe pure nothrow @nogc
{
    ...
    req.minSize = Size(pal.overlayMinWidth, pal.overlayMinHeight);
...
@safe pure nothrow @nogc unittest
{
    ...
    assert(wide.width == pal.overlayMaxWidth, "the ceiling holds");
    assert(wide.height == pal.overlayMaxHeight);
    ...
    assert(tiny.width == pal.overlayMinWidth, "never below the floor");
    assert(tiny.height == pal.overlayMinHeight);
}
...
private uint popupRule(ref Builder b)
    => b.add(Widget(kind: WidgetKind.box, stretch: true,
        ...
        decoration: Decoration(borderWidth: Insets(0, 0, M.borderWeight, 0),
