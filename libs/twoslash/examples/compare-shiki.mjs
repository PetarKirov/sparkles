...
// and is out of scope; docs are rendered as markdown by shiki but escaped text by
// us until the deferred markdown phase, so docs *content* is not compared.
...
const ourCss = join(here, '..', 'src', 'sparkles', 'twoslash', 'views', 'twoslash.css')
const shikiCss = '/home/petar/code/repos/typescript/shiki/packages/twoslash/style-rich.css'
...
const ALLOW_OURS_ONLY = new Set([
  'twoslash-query-line', 'twoslash-meta-line', // our below-line query model
])
...
const classSet = (html) => {
  ...
  for (const m of html.matchAll(/class="([^"]*)"/g))
    for (const c of m[1].split(/\s+/))
      if (c.startsWith('twoslash-')) s.add(c)
  return s
}
...
function ourHtml(fixture) {
  return execFileSync(hue, ['--twoslash', '--html', fixture], { encoding: 'utf8' })
}

if (!existsSync(hue)) {
  console.error(`✗ hue binary not found at ${hue}\n  build it first: dub build :hue`)
  process.exit(2)
}

const sources = readdirSync(join(here, 'src')).filter(f => /\.(ts|tsx)$/.test(f)).sort()
const shikiAll = new Set(), oursAll = new Set()
let checked = 0
...
for (const file of sources) {
  ...
  const shiki = classSet(await shikiHtml(readFileSync(join(here, 'src', file), 'utf8'), langOf(file)))
  const ours = classSet(ourHtml(fixture))
  shiki.forEach(c => shikiAll.add(c))
  ours.forEach(c => oursAll.add(c))
  const missing = [...shiki].filter(c => !ours.has(c) && !ALLOW_SHIKI_ONLY.has(c)).sort()
  const mark = missing.length ? '✗' : '✓'
  console.log(`  ${mark} ${name.padEnd(16)} shiki:${shiki.size} ours:${ours.size}` +
    (missing.length ? `  MISSING: ${missing.join(', ')}` : ''))
  checked++
}

// (a) union vocabulary
const missingClasses = [...shikiAll].filter(c => !oursAll.has(c) && !ALLOW_SHIKI_ONLY.has(c)).sort()
const extraClasses = [...oursAll].filter(c => !shikiAll.has(c) && !ALLOW_OURS_ONLY.has(c)).sort()
...
const missingCss = [...shikiSel].filter(s => !ourSel.has(s) && !ALLOW_SHIKI_ONLY.has(s)).sort()

console.log(`\n(a) HTML class vocabulary over ${checked} fixtures`)
console.log(`    shiki classes: ${shikiAll.size}, ours: ${oursAll.size}`)
console.log(`    missing (shiki has, we don't): ${missingClasses.length ? missingClasses.join(', ') : '— none'}`)
console.log(`    extra   (we have, shiki doesn't): ${extraClasses.length ? extraClasses.join(', ') : '— none'}`)
console.log(`    allowlisted shiki-only: ${[...ALLOW_SHIKI_ONLY].filter(c => shikiAll.has(c)).join(', ') || '—'}`)

console.log(`\n(b) CSS .twoslash-* selector coverage`)
console.log(`    shiki selectors: ${shikiSel.size}, ours: ${ourSel.size}`)
console.log(`    missing rules (shiki styles, we don't): ${missingCss.length ? missingCss.join(', ') : '— none'}`)

console.log(`\nScope: inner syntax tokens excluded (different highlighters); docs content` +
  ` excluded until the markdown phase.`)
