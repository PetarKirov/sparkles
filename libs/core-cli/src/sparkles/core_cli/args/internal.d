...
struct CommandNode(Command_)
{
    ...
    Command value;
    // Back-compat, not decoration: the root command is wrapped in a
    // `CommandNode` now, and this is what keeps `parsedCli.value.logLevel` —
...
///
/// Also recognizes the per-option help shorthand: when the token has no
/// `=` and ends with `?` (and the prefix is non-empty), it's rewritten
/// to `name=<prefix>`, `inlineValue="?"`, `hasInlineValue=true` — so
/// `--color?` and `-c?` flow through the same code path as
/// `--color=?`.
///
/// Only the inline forms are rewritten, and deliberately so. `--option help`
/// and `--option ?` — the space-separated forms — pass through as ordinary
/// values, which is the escape hatch a free-form option needs when `help` or
/// `?` is a value someone legitimately wants to pass.
...
) @safe pure
{
    ...
    else if (token.length > 1 && token[$ - 1] == '?')
    {
        name = token[0 .. $ - 1];
        inlineValue = "?";
        hasInlineValue = true;
...
private CliExpected!void applyOption(string field, T)(
    ref T target,
    Option optionInfo,
    string[] args,
    ref size_t index,
...
)
{
    ...
    static if (is(T == bool))
    {
        if (hasInlineValue)
        {
            if (isOptionHelpToken(inlineValue))
                return error(CliError(
                    kind: CliError.Kind.help,
                    help: formatOptionValueHelp!T(optionInfo, field),
                    exitCode: 0,
                ));
    ...
    else
    {
        ...
        // Per-option help interception: only fires when the value came
        // from the inline `=` form (or the `?`-suffix shorthand, which
        // synthesizes an inline value). For the space-separated form,
        // `help` / `?` pass through as literal values — preserving an
        // escape hatch for free-form string options that legitimately
        // want those literal values.
        if (hasInlineValue)
        {
            foreach (v; values)
                if (isOptionHelpToken(v))
                    return error(CliError(
                        kind: CliError.Kind.help,
                        help: formatOptionValueHelp!T(optionInfo, field),
                        exitCode: 0,
...
private bool isOptionHelpToken(string value) @safe pure nothrow @nogc
{
...
/**
 * Dispatches a leaf command to its handler.
 *
 * `T` is guaranteed to be a leaf: `runParsedCli` only reaches here after every
 * nested `@Subcommands` SumType has been unwrapped. That is what makes the
 * final `static assert(false)` sound rather than over-eager — a command group
 * never arrives, so a missing handler really is a missing handler.
 *
 * It is worth the assert. Before it, a leaf with no `run` was a silent hole:
 * typing `tool somecmd` parsed cleanly, exited 0, and produced nothing at all.
 *
 * The probes call `value.run!Program(program)` — through the *instance*, not
 * `T.run!Program(...)`. An instance receiver resolves to static members too, so
 * existing `static run` handlers keep dispatching, while a leaf may instead
 * declare an instance template and read both `this` and `program` in one body.
 * Rewriting these to the static form compiles and silently drops every
 * instance handler.
 */
private int callRun(Parent, T, Program)(ref T value, ref Program program)
{
    ...
    else static if (__traits(compiles, value.run!Program(program)))
        return invokeReturning0!(() => value.run!Program(program));
    else static if (__traits(compiles, value.run(program)))
        return invokeReturning0!(() => value.run(program));
...
@("args.parseCli.instanceRunTemplateReceivesProgramAndThis")
@system
unittest
{
    @(Command("git"))
    static struct Git
    {
        ...
        static struct Worktree
        {
            ...
            static struct List
            {
                ...
                int run(Program)(in Program program)
                {
                    assert(porcelain, "instance run sees its own parsed `this`");
                    return program.value.directory == "repo" ? 19 : 3;
                }
            }
        }
    ...
    auto parsed = parseCli!Git(["git", "-C", "repo", "worktree", "list", "--porcelain"]);
    assert(parsed);
    assert(runParsedCli(parsed.value) == 19);
}
...
@("args.optionHelp.enum.equalsHelp")
@system
unittest
{
    @(Command("paint"))
    ...
    auto help = parsed.error.help;
    assert(help.canFind("red"));
    assert(help.canFind("green"));
    assert(help.canFind("blue"));
    assert(help.canFind("--color"));
    assert(help.canFind("Pick a color."));
}

@("args.optionHelp.enum.equalsQuestionMark")
@system
unittest
{
    @(Command("paint"))
    ...
    auto parsed = parseCli!Cli(["paint", "--color=?"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    assert(parsed.error.help.canFind("red"));
}

@("args.optionHelp.enum.suffixShorthandLong")
@system
unittest
{
    @(Command("paint"))
    ...
    assert(!parsed);
    assert(parsed.error.isHelp);
    assert(parsed.error.help.canFind("blue"));
}

@("args.optionHelp.enum.suffixShorthandShort")
@system
unittest
{
    @(Command("paint"))
    ...
    auto parsed = parseCli!Cli(["paint", "-c?"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    assert(parsed.error.help.canFind("green"));
}

@("args.optionHelp.enum.shortFormEqualsHelp")
@system
unittest
{
    @(Command("paint"))
    ...
    auto parsed = parseCli!Cli(["paint", "-c=help"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    assert(parsed.error.help.canFind("red"));
}

@("args.optionHelp.allowedValues")
@system
unittest
{
    @(Command("tool"))
    struct Cli
    {
        @(Option("level", allowedValues: ["trace", "info", "warn"]))
        string level;
    }

    auto parsed = parseCli!Cli(["tool", "--level=help"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    auto help = parsed.error.help;
...
unittest
{
    ...
    auto parsed = parseCli!Cli(["tool", "--verbose=help"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    auto help = parsed.error.help;
    assert(help.canFind("true"));
    assert(help.canFind("false"));
}

@("args.optionHelp.freeFormString")
@system
unittest
{
    @(Command("tool"))
    struct Cli
    {
        @(Option("name", description: "Your name."))
        string name;
    }

    auto parsed = parseCli!Cli(["tool", "--name=help"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    auto help = parsed.error.help;
    assert(help.canFind("any string"));
    assert(help.canFind("Your name."));
}

@("args.optionHelp.spaceFormPassesThrough")
@system
unittest
{
    // Universal-scope tradeoff: `--name help` (space form) assigns the
    // literal string "help" instead of triggering the help screen. This
    // is the documented escape hatch for free-form options that legitimately
    // need to accept the literal value.
    @(Command("tool"))
    struct Cli
    {
        @(Option("name"))
    ...
    auto parsed = parseCli!Cli(["tool", "--name", "help"]);
    assert(parsed);
    assert(parsed.value.name == "help");
}

@("args.optionHelp.unknownOptionWithSuffix")
@system
unittest
{
    @(Command("tool"))
    ...
    auto parsed = parseCli!Cli(["tool", "--unknown?"]);
    assert(!parsed);
    assert(!parsed.error.isHelp);
    assert(parsed.error.message.canFind("Unknown option"));
}

@("args.optionHelp.shortCircuitsRequiredCheck")
@system
unittest
{
    // `--color=help` must short-circuit before validateRequired, so the
    // user can discover what `--required-thing` accepts without first
    // having to satisfy every other required option.
    @(Command("tool"))
    struct Cli
    {
        @(Option("color"))
    ...
    auto parsed = parseCli!Cli(["tool", "--color=help"]);
    assert(!parsed);
    assert(parsed.error.isHelp);
    assert(parsed.error.exitCode == 0);
