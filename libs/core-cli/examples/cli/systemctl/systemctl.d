...
struct Start
{
    @(Option(`no-block`, description: "Do not synchronously wait for the requested operation to finish"))
    bool noBlock;
...
struct Enable
{
    @(Option(`now`, description: "Start the unit(s) immediately after enabling them"))
    bool now;

    @(Option(`runtime`, description: "Make changes only temporarily, lost on the next reboot"))
    bool runtime;

    @(Option(`force`, description: "When linking unit files, override existing symlinks"))
    bool force;
...
struct Disable
{
    @(Option(`now`, description: "Stop the unit(s) immediately after disabling them"))
    bool now;
...
struct Mask
{
    @(Option(`now`, description: "Stop the unit(s) immediately after masking them"))
    bool now;
...
struct Status
{
    @(Option(`l|full`, description: "Don't ellipsize unit names or process trees"))
    bool full;

    @(Option(`n|lines`, description: "Number of journal lines to show"))
    int lines = 10;

    @(Option(`no-pager`, description: "Do not pipe output into a pager"))
    bool noPager;

    @(Argument("units", optional: true))
    string[] units;
...
struct IsActive
{
    @(Option(`q|quiet`, description: "Suppress textual output, only signal via exit code"))
    bool quiet;
...
struct ListUnits
{
    @(Option(`a|all`, description: "Show all loaded units regardless of state"))
    bool all;

    @(Option(`reverse`, description: "Show reverse dependencies"))
    bool reverse;
...
{
    @(Option(`a|all`, description: "Show all unit files regardless of enabled-state"))
    bool all;
...
struct Show
{
    @(Option(`p|property`, description: "Show only properties matching the given name. Can be specified multiple times."))
    string[] properties;

    @(Option(`a|all`, description: "Show all properties, including those with empty values"))
    bool all;

    @(Option(`value`, description: "Print only the property values, omitting names"))
    bool value;
...
struct Edit
{
    @(Option(`full`, description: "Edit the full unit file rather than creating an override drop-in"))
    bool full;

    @(Option(`force`, description: "Create the unit file even if it does not exist"))
    bool force;

    @(Option(`runtime`, description: "Apply edits only at runtime, lost on the next reboot"))
    bool runtime;
...
struct Kill
{
    @(Option(`s|signal`, description: "Signal name to send (default: SIGTERM)"))
...
struct Systemctl
{
    @(Option(`system`, description: "Operate on the system service manager (the default when run as root)"))
    bool system_;

    @(Option(`user`, description: "Operate on the user service manager of the calling user"))
    bool user;

    @(Option(`global`, description: "Operate on the global default user-unit configuration"))
    bool global;

    @(Option(`t|type`, allowedValues: unitTypes))
    string type;

    @(Option(`state`, description: "Filter units by load/sub/active state, e.g. 'failed' or 'active,running'"))
    string state;
    ...
    @(Option(`q|quiet`, description: "Suppress informational output, only print errors"))
    bool quiet;
