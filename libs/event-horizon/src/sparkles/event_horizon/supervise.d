...
package alias RelayHandledTestHook = void delegate(RelayKind kind,
    bool cgroupOwned, bool migrated, scope const(char)[] cgroupPath);
...
private struct Run
{
    ...
    version (unittest)
    {
        ...
        RelayHandledTestHook afterHandle;
    }
...
package IoResult!SupervisedProcessResult superviseImpl(ref Sched s,
    ...
    int testObserverErrno = 0,
    scope RelayHandledTestHook afterHandle = null) @trusted
{
    ...
    auto outcome = withScope!((ref sc) {
        ...
        struct Ctl
        {
            ...
            void handle(RelayMessage msg)
            {
                ...
                version (unittest)
                    version (linux)
                        if (rp.afterHandle !is null && rp.control == ControlState.running)
                            rp.afterHandle(msg.kind, rp.cgroupOwned, rp.migrated,
                                rp.cgroup.path[]);
            }
...
unittest
{
    ...
    import core.sys.posix.fcntl : O_CLOEXEC, O_NONBLOCK, O_RDWR, open;
    import core.sys.posix.signal : kill_ = kill;
    import core.sys.posix.stdlib : mkdtemp;
    import core.sys.posix.sys.stat : mkfifo;
    import core.sys.posix.unistd : close, getsid, write_ = write;
    import std.conv : text, to;
    import std.algorithm.searching : canFind;
    import std.file : FileException, exists, readText, remove, rmdir, tempDir;
    import std.path : buildPath;
    import std.process : escapeShellFileName;
    import std.string : indexOf, splitLines, toStringz;
    import sparkles.event_horizon.sampling : ProcStat, parseProcStat;
    import sparkles.event_horizon.scope_ : JoinHandle, withScope;

    Sched s;
    schedOrSkip(s);
    auto template_ = (buildPath(tempDir(), "eh-fragment-XXXXXX") ~ '\0').dup;
    assert(mkdtemp(template_.ptr) !is null);
    const dir = template_[0 .. $ - 1].idup;
    scope (exit) rmdir(dir);
    const gate = buildPath(dir, "fragment");
    const identityFile = buildPath(dir, "identity");
    const pendingIdentity = buildPath(dir, "identity-pending");
    scope (exit) if (exists(identityFile)) remove(identityFile);
    scope (exit) if (exists(pendingIdentity)) remove(pendingIdentity);
    assert(mkfifo(gate.toStringz, 0x180 /* 0600 */) == 0);
    scope (exit) remove(gate);
    const gateFd = open(gate.toStringz, O_RDWR | O_NONBLOCK | O_CLOEXEC);
    assert(gateFd >= 0);
    scope (exit) close(gateFd);
    ...
    void loadIdentity()
    {
        const saved = readText(identityFile);
        ProcStat st;
        assert(parseProcStat(saved, st));
        descendant = saved[0 .. saved.indexOf(' ')].to!int;
    ...
    bool stillLive()
    {
        ...
        {
            ProcStat current;
            assert(parseProcStat(readText(text("/proc/", descendant, "/stat")), current));
            return current.startTime == startTime && !current.zombie;
        }
        catch (FileException e)
        {
            if (e.errno == ENOENT || e.errno == ESRCH)
                return false;
            throw e; // unreadability is not proof of termination
        }
    }
    // Even a failing assertion must release the helper and clean up the
    // separate session. Its saved start time guards against PID reuse.
    scope (exit)
    {
        cast(void) write_(gateFd, "go\n".ptr, 3);
        if (!identityKnown && exists(identityFile))
            loadIdentity();
        if (identityKnown && stillLive())
            cast(void) kill_(descendant, SIGKILL);
    }

    SupervisedProcessResult got;
    bool rootHandled, stderrHandled, ready, released, cancelled, supported;
    const before = MonoTime.currTime;
    auto r = s.run(() {
        auto outcome = withScope!((ref outer) {
            JoinHandle!void h;
            outer.fork(h, () {
                SupervisedProcessConfig cfg;
                cfg.timeout = 5.seconds; // failure bound, never an ordering device
                cfg.outputGrace = 5.seconds;
                cfg.terminateGrace = 100.msecs;
                // stdin is written by a worker admitted AFTER migration.
                // No descendant can fork until the root receives that token.
                const daemon = `trap "" TERM; cat /proc/$$/stat > `
                    ~ escapeShellFileName(pendingIdentity) ~ ` && mv `
                    ~ escapeShellFileName(pendingIdentity) ~ ` ` ~ escapeShellFileName(identityFile)
                    ~ ` && echo ready; exec >/dev/null 2>&1; exec sleep 30`;
                const command = `read start || exit 1; setsid sh -c ` ~ escapeShellFileName(daemon)
                    ~ ` & (read release < ` ~ escapeShellFileName(gate)
                    ~ `; printf partial) 2>/dev/null & exit 0`;
                auto run = superviseImpl(s, ["sh", "-c", command], cfg,
                    cast(const(ubyte)[]) "go\n", (in ProcessEvent ev) {
                        if (ev.kind != ProcessEventKind.line)
                            return;
                        if (ev.line.terminated)
                        {
                            assert(ev.line.bytes == cast(const(ubyte)[]) "ready");
                            loadIdentity();
                            ready = true;
                            return;
                        }
                        assert(rootHandled && stderrHandled && ready && released,
                            "root observation and readiness precede final-fragment cancellation");
                        assert(ev.line.bytes == cast(const(ubyte)[]) "partial");
                        cancelled = true;
                        outer.cancel();
                    }, null, null, 0,
                    (RelayKind kind, bool owned, bool migrated, scope const(char)[] path) {
                        if (kind == RelayKind.rootObserved)
                            rootHandled = true;
                        // stdout is held by the helper until release, so an
                        // earlier EOF can only be stderr. Make stdout the
                        // LAST terminal stream: its callback must issue the
                        // kill in this handler, not a later protected loop.
                        if (kind == RelayKind.streamEof && !released)
                            stderrHandled = true;
                        if (!rootHandled || !stderrHandled || !ready || released)
                            return;
                        supported = owned;
                        if (supported)
                        {
                            assert(migrated, "a supported run must migrate before releasing stdin");
                            assert(readText(text("/proc/", descendant, "/cgroup")).splitLines
                                .canFind("0::" ~ path), "descendant belongs to this exact run");
                            ProcStat st;
                            assert(parseProcStat(readText(text("/proc/", descendant, "/stat")), st));
                            assert(st.startTime == startTime && !st.zombie);
                            assert(st.pgrp == descendant && getsid(descendant) == descendant,
                                "ready was sent after setsid, outside the root's group");
                        }
                        released = true;
                        assert(write_(gateFd, "go\n".ptr, 3) == 3);
                    });
                assert(run.hasValue, run.hasError ? run.error.context : "");
                got = move(run.value);
                return ioOk();
    ...
    assert(!r.hasError);
    assert(rootHandled && ready && released && cancelled);
    if (!supported)
        skipTest("requires an owned cgroup with kill capability; proc-only run cleaned up");
    assert(got.end == ProcessEnd.cancelled);
    assert(got.status.ok && got.reap == ReapOutcome.reaped);
    assert(!got.terminationDegraded, got.terminationError.context);
    assert(MonoTime.currTime - before < 5.seconds);
    // The adopter owns the orphan's reap: a zombie is dead, not a survivor.
    const deadline = MonoTime.currTime + 1.seconds;
    while (stillLive() && MonoTime.currTime < deadline)
        Thread.sleep(1.msecs);
    assert(!stillLive(), "the captured descendant identity is no longer live");
