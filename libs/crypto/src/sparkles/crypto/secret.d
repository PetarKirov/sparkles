 ...
 *   $(LI $(LREF SecretBuffer) — a growable, `UniqueBuffer`-shaped secret
...
/**
 * A growable secret buffer modeled on $(D sparkles.base.buffer.UniqueBuffer).
 *
 * Elements are stored inline up to `N`, then on the heap via $(D pureMalloc)
 * when capacity is exceeded. Unlike `UniqueBuffer`, this type wipes its
