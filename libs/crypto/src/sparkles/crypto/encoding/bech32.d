...
attributes are *inferred* from `Writer`: `@nogc` with a `UniqueBuffer`, but
...
Params:
    ...
    w    = output range of `char` (e.g. a `UniqueBuffer!(char, N)`)
*/
...
// its `Writer` output range: `@nogc` when handed a `UniqueBuffer`, but
...
unittest
{
    ...
    UniqueBuffer!(char, 128) corrupted;
    const src = buf[];
    foreach (i, c; src)
    {
...
unittest
{
    ...
    UniqueBuffer!(char, 128) lower;
    encodeBech32("age", payload[], lower);

    UniqueBuffer!(char, 128) upper;
    foreach (c; lower[])
