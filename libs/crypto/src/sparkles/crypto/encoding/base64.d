 ...
 * $(REF ParseExpected, sparkles,base,text,errors)`!(ubyte[])` whose value
 ...
 *        $(REF nonCanonicalEncoding, sparkles,base,text,errors);)
 *   $(LI an unpadded length of `1 mod 4` is impossible
 *        ($(REF nonCanonicalEncoding, sparkles,base,text,errors));)
 *   $(LI $(LREF decodeBase64) rejects every `=`
 *        ($(REF invalidPadding, sparkles,base,text,errors)); the padded
...
// when handed a `UniqueBuffer`, but GC-using when the age layer hands them a
    ...
    @safe pure nothrow
{
    ...
    // can't bind to `UniqueBuffer.put`'s `auto ref char`; emit a plain `char`.
...
// attributes are *inferred* from the `Writer`: `@nogc` with a `UniqueBuffer`,
...
/// $(REF invalidPadding, sparkles,base,text,errors); a non-alphabet
/// character as $(REF unexpectedCharacter, sparkles,base,text,errors)
/// (at its offset); a partial group's non-zero unused bits, or a `1 mod 4`
/// length, as $(REF nonCanonicalEncoding, sparkles,base,text,errors).
...
/// $(REF invalidPadding, sparkles,base,text,errors). The remaining rules
