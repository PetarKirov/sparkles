...
if (is(O == Mapped!(T, Just!Origin)) && is(S == Mapped!(T, Nullable)))
{
    static foreach (i, _; T.init.tupleof)
    {
        ...
        else if (!overlay.tupleof[i].isNull)
        {
...
Sparse!T mergeSparse(T)(Sparse!T base, Sparse!T deltas) @safe
{
    ...
    static foreach (i, _; base.tupleof)
    {
        ...
        else if (!deltas.tupleof[i].isNull)
...
@system unittest
{
    ...
    auto part = `{"theme":"dark","pane":{"tabWidth":8}}`.fromJSON!(Sparse!Cfg);
    ...
    auto text = part.value.toJSON;
    assert(!text.hasError);
    auto back = text.value[].fromJSON!(Sparse!Cfg);
...
@safe unittest
{
    ...
    resolved.applyOverlay(origins, cli, Layer.cli);
...
@safe unittest
{
    ...
    resolved.applyOverlay(origins, Sparse!Cfg.init, Layer.user);
