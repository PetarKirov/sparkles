...
private void finalizeChildren(Allocator)(ref SdlDocument!Allocator document)
{
    ...
    {
        const parentDepth = document.nodes[i].depth == 0
            ? uint.max : document.nodes[i].depth - 1;
        size_t parent = i;
        while (parent != 0)
        {
            parent--;
            if (document.nodes[parent].depth == parentDepth)
            {
                document.nodes[parent].childCount++;
                break;
            }
        }
    }
    ...
    foreach (i; 1 .. document.nodes.length)
    {
        ...
        while (parent != 0)
        {
            ...
            if (document.nodes[parent].depth == parentDepth)
            {
                auto slot = document.nodes[parent].childStart
                    + document.nodes[parent].childCount++;
                document.childIndexes[slot] = i;
                break;
            }
        }
    }
