...
if (is(T == float) || is(T == real))
{
    ...
    // One guard digit beyond max_digits10 works around Phobos' x87
    // min-normal formatting boundary; the search still returns the first
        ...
        foreach (precision; 1 .. maxDigits + 1)
        {
            ...
            try
            {
                static if (is(T == float))
                    roundTrips = cast(float) candidate[].to!double is value;
                else
                    roundTrips = candidate[].to!T is value;
            }
            catch (ConvException)
                continue;
            if (roundTrips)
            {
                writeFixedDecimal(writer, candidate[]);
                put(writer, suffix);
                return sdlOk();
            }
        }
...
version (unittest)
{
        ...
        scope const(char)[] token)
    {
        ...
        case decimal:
            assert(parseFixedHarness!real(token[0 .. $ - 2])
