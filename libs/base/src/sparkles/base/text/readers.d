...
///
/// Expressed on the same reverse table as `readInteger!(T, 16)` — a hex
/// digit is exactly a symbol whose `alnum` value is below 16 — so the two
/// cannot drift apart.
bool isHexDigit(char c)
    => cast(uint) alnumDecodeTable[c] < 16;

/// The value `0 … 15` of a hex digit `c` (case-insensitive). `c` must satisfy
/// $(LREF isHexDigit).
ubyte hexNibble(char c)
in (isHexDigit(c))
    => cast(ubyte) alnumDecodeTable[c];
