...
struct Alphabet
{
    ...
    /**
    `true` iff the vocabulary is self-consistent — i.e. $(LREF
    makeDecodeTable) can derive an unambiguous reverse table from it:

    $(UL
        $(LI 2 … 255 digits, none repeated;)
        $(LI under `caseInsensitive`, no digit whose opposite-case twin is
            also a digit (setting it on an alphabet like `base64`, which
            uses both cases as distinct symbols, would silently clobber
            half the table);)
        $(LI a `padding` character that is not itself a digit (the decoder
            tests for padding before consulting the table, so a padding
            character in the vocabulary would be unreachable as a symbol);)
        $(LI `aliases` in whole pairs, each mapping a non-digit onto a
            digit (an alias whose target is not a digit resolves to `-1`
            and silently does nothing).)
    )

    Checked by a `static assert` wherever an `Alphabet` is bound as a
    template parameter, so a malformed vocabulary is a compile error at the
    point of use rather than a wrong answer at run time.
    */
    bool isConsistent() const @safe pure nothrow @nogc
    {
        if (digits.length < 2 || digits.length > 255)
            return false;

        bool[256] isDigit;
        foreach (char c; digits)
        {
            if (isDigit[cast(ubyte) c])
                return false;
            isDigit[cast(ubyte) c] = true;
        }
        if (caseInsensitive)
            foreach (char c; digits)
                if (caseFlip(c) != c && isDigit[cast(ubyte) caseFlip(c)])
                    return false;
        if (padding != '\0' && (isDigit[cast(ubyte) padding]
                || (caseInsensitive && isDigit[cast(ubyte) caseFlip(padding)])))
            return false;
        if (aliases.length % 2 != 0)
            return false;
        for (size_t i = 0; i < aliases.length; i += 2)
        {
            immutable char from = aliases[i], to = aliases[i + 1];
            if (!isDigit[cast(ubyte) to])
                return false;
            if (isDigit[cast(ubyte) from]
                    || (caseInsensitive && isDigit[cast(ubyte) caseFlip(from)]))
                return false;
        }
        return true;
    }
...
/// The opposite-case twin of an ASCII letter; identity for anything else.
private char caseFlip(char c) @safe pure nothrow @nogc
    => c >= 'A' && c <= 'Z' ? cast(char)(c + 32)
        : c >= 'a' && c <= 'z' ? cast(char)(c - 32) : c;

/// The `static assert` message for a vocabulary that fails
/// $(LREF Alphabet.isConsistent).
private enum malformedAlphabet = "Alphabet is not self-consistent: check for "
    ~ "duplicate digits, a padding character that is also a digit, "
    ~ "caseInsensitive on an alphabet that uses both letter cases as "
    ~ "distinct symbols, or a malformed alias pair";

/// Greatest common divisor, for the `charsPerGroup` derivation.
///
/// Hand-rolled rather than `std.numeric.gcd`: that module imports
/// `std.complex` (for its FFT), which is not `-betterC`-compatible, so
/// pulling it into this one would break `@betterC` builds of every consumer
/// of `sparkles.base.text`.
private size_t gcd(size_t a, size_t b) @safe pure nothrow @nogc
{
    while (b)
    {
        immutable t = a % b;
...
byte[256] makeDecodeTable(const Alphabet a) @safe pure nothrow @nogc
{
    ...
    // `caseFlip` is the identity for non-letters, so the fold pass is a
    // harmless re-write for those rather than needing a range test.
    if (a.caseInsensitive)
        foreach (i, char c; a.digits)
            t[cast(ubyte) caseFlip(c)] = cast(byte) i;
    for (size_t i = 0; i + 1 < a.aliases.length; i += 2)
    {
        immutable byte canon = t[cast(ubyte) a.aliases[i + 1]];
        t[cast(ubyte) a.aliases[i]] = canon;
        if (a.caseInsensitive)
            t[cast(ubyte) caseFlip(a.aliases[i])] = canon;
    }
    return t;
}
...
{
    static assert(a.isConsistent, malformedAlphabet);
...
@("text.base_codecs.Alphabet.isConsistent")
@betterC
unittest
{
    // Every shipped preset is well-formed.
    static assert(alnum.isConsistent);
    static assert(base16.isConsistent);
    static assert(base32.isConsistent);
    static assert(base32hex.isConsistent);
    static assert(base64.isConsistent);
    static assert(base64url.isConsistent);
    static assert(zbase32.isConsistent);

    // Too few digits.
    static assert(!Alphabet(digits: "0").isConsistent);
    // A repeated digit makes the reverse table ambiguous.
    static assert(!Alphabet(digits: "0102").isConsistent);
    // caseInsensitive on an alphabet that uses both cases as distinct
    // symbols would silently clobber half the table — base64's real trap.
    static assert(!Alphabet(digits: base64.digits, caseInsensitive: true).isConsistent);
    // A padding character inside the vocabulary is unreachable as a symbol
    // (the decoder tests for padding first).
    static assert(!Alphabet(digits: "01=3", padding: '=').isConsistent);
    static assert(!Alphabet(digits: "01a3", caseInsensitive: true, padding: 'A')
            .isConsistent);
    // Alias pairs: whole pairs, non-digit onto digit.
    static assert(!Alphabet(digits: "0123", aliases: "O").isConsistent);
    static assert(!Alphabet(digits: "0123", aliases: "O9").isConsistent); // '9' is not a digit here
    static assert(!Alphabet(digits: "0123", aliases: "20").isConsistent); // '2' already is one
}
