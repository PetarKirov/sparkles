...
A fence taller than the golden threshold shows a fixed-height vertical
viewport with its own track:
...
struct Build
{
    ...
    @(Option("combined", description: "Tries to build the whole project in a single compiler run"))
    bool combined;

    @(Option("rdmd", description: "Use rdmd instead of directly invoking the compiler"))
    bool rdmd;

    @(Option(`build-mode`, allowedValues: ["separate", "allAtOnce", "singleFile"]))
    string buildMode = "separate";

    @(Option("temp-build", description: "Builds the project in the temp folder if possible"))
    bool tempBuild;

    @(Argument("package", optional: true))
    string packageName;
...
struct Run
{
    ...
    @(Option("temp-build", description: "Builds in temp directory and runs from there"))
    bool tempBuild;

    @(Argument("package", optional: true))
    string packageName;

    @(Argument("program-args", optional: true))
    string[] programArgs;
...
struct Test
{
    ...
    @(Option("parallel", description: "Runs multiple compiler instances in parallel, if possible"))
    bool parallel_;

    @(Option("test", description: "Execute the built test binary after compilation. Pass --no-test to compile without running, e.g. for cross-compilation pipelines."))
    bool test_ = true;

    @(Option("coverage", description: "Enables code coverage statistics to be generated"))
    bool coverage;

    @(Option("coverage-ctfe", description: "Enables code coverage (including CTFE) statistics to be generated"))
    bool coverageCtfe;

    @(Option("main-file", description: "Specifies a custom file containing the main() function to use for running the tests"))
    string mainFile;
...
struct Clean
{
    @(Option("all-packages", description: "Cleans all known packages, regardless of whether they are used by the current package or not"))
    bool allPackages;
...
struct Fetch
{
    @(Option(`r|recursive`, description: "Also fetches dependencies of specified packages"))
    bool recursive;
...
struct Add
{
    @(Option("recipe", description: "Override path to recipe file (dub.sdl/dub.json)"))
    string recipe;
...
struct Remove
{
    @(Option(`n|non-interactive`, description: "Don't enter interactive mode"))
    bool nonInteractive;
...
struct Upgrade
{
    @(Option(`prerelease`, description: "Uses the latest pre-release version, even if release versions are available"))
    bool prerelease;

    @(Option(`s|sub-packages`, description: "Also upgrades dependencies of all directory based sub packages"))
    bool subPackages;

    @(Option(`verify`, description: "Updates the project and performs a build; if successful, rewrites the selected versions file"))
    bool verify;

    @(Option(`dry-run`, description: "Only print what would be upgraded, but don't actually upgrade anything"))
    bool dryRun;

    @(Argument("packages", optional: true))
    string[] packages;
...
struct Describe
{
    ...
    @(Option("data-list", description: "Output --data information separated by newlines instead of spaces"))
    bool dataList;
...
{
    return runCli!Dub(args);
}
```

Prose after the tall fence.
