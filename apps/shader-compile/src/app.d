...
loudly, exit 0 — when that compiler is not on the machine, which is what CI
sees. Point `$SPARKLES_SHADER_LDC` (or `--ldc`) at an LDC built from the
...
eventually. Run it from the repository root (`dub run :shader-compile`).
*/
module app;
...
immutable Unit[] units = [
    Unit(
        ...
        outDir: "libs/ui/shaders/generated",
    ),
];
...
int run(in Unit unit, string ldc, bool verify, bool keep, bool quiet)
{
    ...
    if (compiled.status != 0)
    {
        if (isToolchainAbsent(compiled))
        {
            stderr.writefln("shader-compile: %s: skipped — no dcompute-enabled LDC (`%s`); " ~
                "set $SPARKLES_SHADER_LDC to a `sparkles/vulkan-shaders` build. %s",
    ...
    foreach (name; produced.keys.sort)
    {
        ...
        if (verify)
        {
            ...
            {
                stderr.writefln("shader-compile: %s: DRIFT — regenerate with `dub run :shader-compile`", target);
                drift = 1;
            }
        }
        else
...
private string header(in Unit unit, string entry)
    => format("// Generated by `dub run :shader-compile` from %s (entry point `%s`).\n" ~
