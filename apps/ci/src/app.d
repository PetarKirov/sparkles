...
nix run .#ci -- --smoke-apps [--smoke-frames N] [--smoke-timeout MS] [--smoke-bin-dir DIR] [--include APP...]
...
$(LIST
    ...
    $(ITEM `--include-files` (aliases `--files`, `--include`) — select explicit files or git-style globs; when omitted, each mode uses its tracked defaults)
...
struct CliParams
{
    ...
    // Canonical name last (same convention as short|long): `--files` and
    // `--include` are aliases, `--include-files` is the primary name shown in
    // synopsis/errors.
    @(Option(`files|include|include-files`,
        description: "File paths or git-style globs to include — or, for "
            ~ "--smoke-apps, application names. Pass one or more selectors "
            ~ "immediately after the flag. Aliases: --files, --include. When "
            ~ "omitted, each mode uses its tracked defaults."))
    string[] files;
...
)
{
    ...
    if (cli.smokeApps && (cli.verify || cli.update || cli.exampleFiles || cli.build || cli.test
            || cli.testSanitize || cli.testExtracted || cli.dedupReferenceLinks
            || cli.fixReferenceLinks || cli.checkCommitScope || cli.checkVcsUrls
            || cli.checkDocsSidebar || cli.checkBlobPaths || cli.ciStats
            || cli.auditFences || cli.mirrorChecks))
    ...
    if (positionalArgs.length > 0 && !cli.checkCommitScope)
        return "Positional file arguments are no longer supported; use --include-files (aliases --files, --include)";
...
/// when the two agree.
/// Runs the application smoke check (`--smoke-apps`). `--include` selects a
