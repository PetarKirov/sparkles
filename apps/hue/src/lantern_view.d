...
$(MREF lantern) decides $(I what) is pending; this decides what that looks
like. Nothing here draws: it appends a subtree to a
$(REF Builder, sparkles,ui,widget), which each backend already knows how to
paint — so the GUI, the TUI and (for a static cheat sheet) HTML get one panel
...
import keymap : Binding, Chord, ShiftReq;
...
which-key ships three presets; hue ships the two whose difference is real. Both
anchor to the bottom, because that is the edge a reader is least likely to be
reading.
*/
...
struct BoxLayout
{
    ...
    int capacity() const pure nothrow @nogc => boxes * rows;
}
...
BoxLayout packBoxes(int count, int avail, int contentWidth,
    in LanternStyle st = LanternStyle.init) pure nothrow @nogc
{
    BoxLayout l;
    if (count <= 0 || avail <= 0)
        return l;
...
the panel too, as which-key and `CFG6`'s proposed spelling both have it.
*/
private Span writeKeyLabel(ref LabelArena arena, in Chord c) nothrow @nogc
{
...
private void appendChar(ref LabelArena arena, dchar ch) nothrow @nogc
{
    if (ch == ' ')
    {
        // A word, like the arrows above: the leader is the one key the whole
        // map hangs off, and `␣` renders as an underscore in several fonts.
        arena ~= "Space";
        return;
    }
    if (ch < 0x80)
    ...
    // The table's own code points are ASCII today; encode anything else so a
    // configured binding (`CFG6`) cannot render as mojibake.
    char[4] buf;
    size_t n;
    if (ch < 0x800)
    {
...
private string namedKeyLabel(Key k) pure nothrow @nogc
{
    final switch (k)
    {
...
private void measure(scope const Binding[] items, size_t depth,
    ref LabelArena arena, out int keyWidth, out int descWidth) nothrow @nogc
{
...
/// Cells a UTF-8 label occupies. One per code point: hue's own labels are
/// ASCII plus a handful of BMP arrows, and the true width table is
/// [`DEF7`](../../docs/specs/hue/feature-requirements.md), still deferred.
private size_t displayWidth(scope const(char)[] s) pure nothrow @nogc
{
    size_t n;
    foreach (c; s)
        if ((c & 0xC0) != 0x80)
...
`items` is what $(REF bindingsAt, keymap) listed, `depth` the number of chords
already typed (so the label comes from the right chord of each path), and
`arena` a caller-owned buffer this fills with the labels the tree borrows.
...
it — the defect `WGT15`'s docs describe.
*/
uint viewLantern(ref Builder b, ref LabelArena arena,
    scope const Binding[] items, size_t depth, int availWidth,
...
private uint itemCell(ref Builder b, const(char)[] key, const(char)[] desc,
    bool isGroup, int keyWidth, int cellWidth, in LanternStyle st, size_t hitId)
{
...
private const(char)[] separatorText(dchar sep) pure nothrow @nogc
{
    return sep == '→' ? "→" : sep == '➜' ? "➜" : sep == '>' ? ">" : "→";
}
...
@("lantern_view.boxesFillTheWidth")
...
@("lantern_view.keyLabelsAreReadable")
@safe nothrow @nogc
unittest
{
    LabelArena a;
    import keymap : chord, chordRange;
    ...
    // The leader must be visible. A blank cell reads as "no binding", which
    // is exactly wrong for the one key the whole map hangs off.
    assert(lbl(chord(' ')) == "Space");
}

@("lantern_view.everyLabelSurvivesTheArenaGrowing")
@safe
unittest
{
    import keymap : bindingsAt, KeyContext;
    ...
    //
    // The fixture is the real table, which is well past the arena's inline
    // capacity, so this exercises at least one grow rather than assuming one.
    SmallBuffer!(Binding, 128) listed;
    bindingsAt(listed, KeyContext.init);
    assert(listed.length > 10, "the fixture needs enough rows to force a grow");

    LabelArena arena;
    Builder b;
    BoxLayout layout;
    const root = viewLantern(b, arena, listed[], 0, 120, layout);
    auto tree = b.finish(root);

    // Every text run is non-empty and matches a label that was written — the
    // observable form of "no slice went stale".
    const shown = layout.capacity < cast(int) listed.length
        ? layout.capacity : cast(int) listed.length;
    ...
    foreach (ref w; tree.nodes)
    {
        ...
        if (w.text.length >= listed[0].desc.length
            && w.text[$ - listed[0].desc.length .. $] == listed[0].desc)
            sawFirst = true;
        const last = listed[shown - 1].desc;
    ...
    if (layout.capacity < cast(int) listed.length)
    {
        Builder b2;
        LabelArena arena2;
        BoxLayout l2;
        const r2 = viewLantern(b2, arena2, listed[], 0, 120, l2,
            Placement.classic, LanternStyle.init, 0,
            cast(int) listed.length);
        auto t2 = b2.finish(r2);
        bool sawLast;
        foreach (ref w; t2.nodes)
            if (w.kind == WidgetKind.text
                && w.text == listed[listed.length - 1].desc)
                sawLast = true;
        assert(sawLast, "the last binding must be reachable by scrolling");
    }
}

@("lantern_view.columnsDoNotOverlap")
@safe
unittest
{
    import keymap : bindingsAt, KeyContext;
    ...
    Builder b;
    BoxLayout box;
    const root = viewLantern(b, arena, listed[], 0, 120, box);
    auto tree = b.finish(root);
...
@("lantern_view.groupsAreMarkedAndCommandsAreNot")
@safe
unittest
{
    import keymap : bindingsAt, KeyContext;
    ...
    // `z` and `<leader>` are prefixes, so their labels carry the `+` that
    // tells a reader more keys follow — the one thing distinguishing "this
    // runs something" from "this opens a menu" before you press it.
