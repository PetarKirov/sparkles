...
import agents;
import bump;
import conventional : CommitType;
import git;
import notes;
import preflight : runPreflight, PreflightResult;
...
private Result!ReleaseStats buildStats(Commit[] commits, string fromRef)
{
    import result : success, failure;

    ReleaseStats rs;
    rs.commitCount = commits.length;
    rs.tally = tallyCommits(commits);
    rs.typeCounts = typeCounts(commits);
    rs.breakingCount = rs.tally.breaking;

    auto ds = diffStat(fromRef, "HEAD");
    if (ds.hasValue)
    {
        rs.filesChanged = ds.value.filesChanged;
        rs.insertions = ds.value.insertions;
        rs.deletions = ds.value.deletions;
    }

    auto authors = authorCounts(fromRef, "HEAD");
    if (authors.hasValue)
        rs.authors = authors.value;

    auto files = numstat(fromRef, "HEAD");
    if (files.hasValue)
        rs.areas = areaBreakdown(files.value, expandableAreas);
...
private Nullable!NotesMode resolveNotesMode(in CliParams cli)
{
...
private string verStr(in SemVer v)
{
...
private string bumpName(BumpKind k)
{
    final switch (k)
    {
...
private string agentKeys(const(AgentSpec)[] specs)
{
