...
import sparkles.release.stats : Commit, AuthorCount, FileStat;
...
private Result!string git(const(string)[] args, string workDir = null)
{
    ...
    if (r.status != 0)
    {
        ...
        return failure!string(
            styledText(i"`$(cmd)` failed: $(r.stderr.strip.idup)"));
    }
...
Result!(TaggedVersion[]) listTags()
{
    ...
    auto tags = res.value
        ...
        .map!(tag => tuple(tag, SemVer.parseLoose(tag)))
        .filter!(t => t[1].hasValue)
        .map!(t => TaggedVersion(tag: t[0], version_: t[1].value))
        .array;
    return success(tags);
}
...
Nullable!TaggedVersion latestTag(TaggedVersion[] tags) @safe pure nothrow
{
    ...
    return nullable(tags.maxElement!(t => t.version_));
}
...
Result!(Commit[]) logRange(string from, string to)
{
    ...
    foreach (rec; res.value.split("\x1e"))
    {
        ...
        commits ~= Commit(
            ...
            subject: f[3],
            body_: f[4],
            conv: parseConventional(f[3], f[4]),
        );
    }
...
Nullable!FileStat parseNumstatLine(scope const(char)[] line) @safe pure nothrow
{
    ...
    return nullable(FileStat(
        insertions: binary ? 0 : allDigits(insTok),
        deletions: binary ? 0 : allDigits(delTok),
        path: path.idup,
        binary: binary,
    ));
}
    ...
    @safe pure nothrow
{
    ...
    while (i < t.length && (t[i] == ' ' || t[i] == '\t'))
        i++;
    return nullable(AuthorCount(name: t[i .. $].idup, commits: count));
}
...
{
    auto tags = [
        TaggedVersion(tag: "v0.1.0", version_: SemVer(major: 0, minor: 1, patch: 0)),
        TaggedVersion(tag: "v0.10.0", version_: SemVer(major: 0, minor: 10, patch: 0)),
        TaggedVersion(tag: "v0.2.0", version_: SemVer(major: 0, minor: 2, patch: 0)),
    ];
