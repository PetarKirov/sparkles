...
| `--bench`             | Measure `@benchmark` tests (serial). Cases are registered from all bench bodies, then scheduled and measured grouped by their streaming key — the `--group-by` group, else the source test — so each table prints the moment its group finishes, with timing and metric columns aligned on the decimal point. On an interactive terminal a live progress spinner (`⠹ 12/40 name`) animates on stderr; it is auto-suppressed when piped, non-tty, `--no-colours`, `$NO_COLOR`, or `TERM=dumb` (results on stdout stay byte-stable) |
