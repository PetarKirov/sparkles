...
| `--bench`           | Measure `@benchmark` tests (serial); print one statistics table per test as it completes. On an interactive terminal a live progress spinner (`⠹ 12/40 name`) animates on stderr; it is auto-suppressed when piped, non-tty, `--no-colours`, or `$NO_COLOR` (results on stdout stay byte-stable) |
...
| `--sort-by=KEY`     | With `--bench`: sort rows by `name` or a metric column name (ascending). Default: `median/iter`. Applied within `--group-by` groups                                                                                                                                                              |
| `--group-by=SEGS`   | With `--bench`: split the report into one table per group of the given 0-based, `/`-delimited name segments (each titled `benchmark: <group>`, stub column labelled `implementation:` with the group prefix dropped). E.g. `=1,2` groups `engine/dataset/op` by `(dataset, op)`                  |
