...
the `Buffer` family and text parsing primitives. `base` is the smaller layer:
...
- stack-first output storage via the `Buffer` family;
...
needs styled IES rendering on its hot path. Moving only the buffers and
