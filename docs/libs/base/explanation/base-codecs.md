...
Everything else a vocabulary can get wrong is caught by `isConsistent`, a
CTFE predicate `static assert`ed wherever an `Alphabet` binds — duplicate
digits, a padding character that is also a digit, an alias pair whose
target is not a digit, and `caseInsensitive` on an alphabet that already
uses both letter cases as distinct symbols. These all used to fail
_silently_, by producing a reverse table that quietly disagreed with the
encoder; making them compile errors is the same move `percent.d` makes for
`PercentSet.isConsistent`, and the reason both predicates are members
rather than free functions is that the package re-exports both modules
into one namespace.
