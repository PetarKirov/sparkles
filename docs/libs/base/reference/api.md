...
| `sparkles.base.text.float_conv`  | Exact decimal ⇄ binary floating point for any format — `float`, `double`, and `real` at binary64, x87 or binary128: correctly-rounded `readDecimalFloat!T` (Clinger, Eisel–Lemire at 64 and 128 bits, an exact big-decimal tier), shortest round-trip `shortestDigits`/`writeShortest` (Steele–White) and the Schubfach `formatShortestDouble`, over `BinaryFloatFormat`, the format as data ([spec](../../../specs/base/text/float-conv.md)). |
