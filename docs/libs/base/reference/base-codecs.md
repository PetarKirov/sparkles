...
`isConsistent` is the well-formedness predicate, asserted at compile time
wherever an `Alphabet` is bound: 2–255 digits with no duplicates, a
`padding` character that is not itself a digit, `aliases` in whole
non-digit→digit pairs, and — under `caseInsensitive` — no digit whose
opposite-case twin is also a digit. That last one is the real trap:
`caseInsensitive: true` on a `base64`-style alphabet, which uses `A` and
`a` as _distinct_ symbols, would silently clobber half the reverse table.
It is now a `static assert` instead.
...
UniqueBuffer!(char, 64) text;
encodeBase64(text, "Man".representation);
assert(text[] == "TWFu");

UniqueBuffer!(ubyte, 64) bytes;
...
- **Streaming** — `encodeBase!a(w, data)` writes chars to any output range;
  ...
  `UniqueBuffer` writer both are `@safe pure nothrow @nogc`.
- **Fixed-length** — when the byte count `N` is compile-time-known:
  ...
  Note that `encodedLen` is **not injective** for a padding alphabet —
  `encodedLen(base64, 1)`, `…(base64, 2)` and `…(base64, 3)` are all 4 — so
  the fixed-length decoder's constraint cannot pin the payload to exactly
  `N` bytes. A text that carries fewer (`"Zg=="` into a `ubyte[3]`) or more
  (`"TWFu"` into a `ubyte[1]`) is a `widthMismatch` error, not an assert.
...
On failure the streaming decoder's writer — and the fixed-length decoder's
`dst` — may already hold a partial prefix; only the bounds are guaranteed.
Callers needing all-or-nothing semantics decode into a throwaway buffer.
...
Lines are at most `width` chars (which must be positive); no trailing
newline is emitted. The writer takes both a single-character and a bulk
`put`, so a run is forwarded whole rather than element-wise. (This wraps
codec output — single-column ASCII. Prose wrapping by terminal cell width
lives in `sparkles.base.text.wrap`.)
