// ImportC shim for the F05 loop-wakeup demo — the scaffold shim
// (../scaffold/c.c) plus the fd-multiplexing pieces the demo needs:
// <sys/eventfd.h> (the cross-thread wakeup channel), <sys/timerfd.h> (the
// arbitrary-fd probe), <poll.h> (the canonical prepare_read/poll pump) and
// <time.h> (nanosleep, the producer thread's cadence clock) — all inside the
// nogc/nothrow pragma so D callbacks keep their attributes. NOTE: <pthread.h>
// itself is NOT importable — glibc 2.42's pthread.h drags in <linux/types.h>,
// whose `__int128` typedefs ImportC rejects ("__int128 not supported"); the
// demo uses druntime's core.sys.posix.pthread declarations instead.
// Three jobs:
//
...
//   2. Provide the POSIX pieces: shared memory (`memfd_create`, `mmap`,
//      `ftruncate`, `close`) for wl_shm, plus eventfd/timerfd/poll/pthread.
