// ImportC shim for the F06 keyboard demo — the scaffold shim (../scaffold/c.c)
// plus: the wl_keyboard request/listener helpers, libxkbcommon (keymap
// compilation, xkb_state, keysym names, UTF-8, dead-key compose), <poll.h>
// for the prepare_read/poll pump, and <sys/timerfd.h> for the CLIENT-SIDE
// key-repeat timer (on Wayland, repeat is the app's job — wl_keyboard only
// sends repeat_info). All xkbcommon entry points are real exported functions
// (no `static inline`), so they import directly; only the libwayland-client
// inline request helpers need wsi_* re-exports. Three jobs:
//
...
//      `ftruncate`, `close`) — used both for wl_shm buffers and to map the
//      wl_keyboard.keymap fd — plus poll/timerfd and libxkbcommon.
...
/* wl_keyboard.release exists since v3; the demo only binds seats >= v4
 * (repeat_info), so the destructor request is always available. */
void wsi_keyboard_release(struct wl_keyboard *k)
{
