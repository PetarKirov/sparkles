// ImportC shim for the F03/F04 feature demos — the scaffold shim
// (../scaffold/c.c) plus the xdg_toplevel state requests the demos exercise
// (set_fullscreen / unset_fullscreen / set_minimized) and <poll.h> for the
// poll-based event loop F04 uses as an independent clock. Three jobs:
//
...
{
    xdg_toplevel_set_fullscreen(t, NULL); /* NULL output: the compositor picks */
}
