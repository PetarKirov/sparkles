// ImportC shim for the F12 cursor demo (see ./app.d and ../../scaffold.md).
// Same three jobs as the scaffold's shim — one translation unit for the
// libwayland ABI + the scanner-generated glue (xdg-shell, cursor-shape-v1 and
// the tablet-v2 interface table it references, see ./generate.sh), POSIX shm
// pieces, and `wsi_*` re-exports of every `static inline` request helper
// ImportC cannot call. libwayland-cursor's functions (wl_cursor_theme_load &
// co.) are *real* exported symbols, so <wayland-cursor.h> needs no wrappers.
...
void wsi_pointer_set_cursor(struct wl_pointer *p, uint32_t serial,
    struct wl_surface *surface, int32_t hotspot_x, int32_t hotspot_y)
{
    wl_pointer_set_cursor(p, serial, surface, hotspot_x, hotspot_y);
}
...
/* ---- cursor-shape-v1 (generated glue) -------------------------------- */

struct wp_cursor_shape_device_v1 *wsi_cursor_shape_get_pointer(
    struct wp_cursor_shape_manager_v1 *m, struct wl_pointer *p)
{
    return wp_cursor_shape_manager_v1_get_pointer(m, p);
}

void wsi_cursor_shape_manager_destroy(struct wp_cursor_shape_manager_v1 *m)
{
    wp_cursor_shape_manager_v1_destroy(m);
}

void wsi_cursor_shape_set_shape(struct wp_cursor_shape_device_v1 *dev,
    uint32_t serial, uint32_t shape)
{
    wp_cursor_shape_device_v1_set_shape(dev, serial, shape);
}

void wsi_cursor_shape_device_destroy(struct wp_cursor_shape_device_v1 *dev)
{
...
// module names; see `excludedSourceFiles` in ./dub.sdl). tablet-v2 is here
// only because cursor-shape-v1's interface table references
