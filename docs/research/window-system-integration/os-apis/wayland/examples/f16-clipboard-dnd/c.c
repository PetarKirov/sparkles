// ImportC shim for the F16 clipboard + DnD demo (see ./app.d and
// ../../f16-clipboard-dnd.md). The scaffold shim (../scaffold/c.c) plus:
//
//   - wl_data_device_manager / wl_data_source / wl_data_device /
//     wl_data_offer helpers — the SHARED machinery behind both the
//     clipboard (set_selection) and drag-and-drop (start_drag), all core
//     protocol, bound at v3 for the dnd-action negotiation
//     (set_actions / action / finish),
//   - wl_seat / wl_pointer / wl_keyboard helpers (selection offers are
//     delivered on keyboard-focus enter; the drag serial comes from a
//     pointer button press),
//   - <poll.h> + pipe2/read/write for the fd-based data transfer (both
//     directions: serve `send` events, drain `receive` pipes),
...
#define _GNU_SOURCE 1 /* memfd_create(2), pipe2(2) */
...
int wsi_pipe2_cloexec(int fds[2])
{
...
/* ---- wl_data_device_manager (clipboard + DnD, core protocol) --------- */

struct wl_data_device *wsi_ddm_get_data_device(struct wl_data_device_manager *m,
    struct wl_seat *seat)
{
    return wl_data_device_manager_get_data_device(m, seat);
}

struct wl_data_source *wsi_ddm_create_data_source(struct wl_data_device_manager *m)
{
    return wl_data_device_manager_create_data_source(m);
}

void wsi_ddm_destroy(struct wl_data_device_manager *m)
{
    wl_data_device_manager_destroy(m);
}

int wsi_data_source_add_listener(struct wl_data_source *s,
    const struct wl_data_source_listener *l, void *data)
{
    return wl_data_source_add_listener(s, l, data);
}

void wsi_data_source_offer(struct wl_data_source *s, const char *mime)
{
    wl_data_source_offer(s, mime);
}

void wsi_data_source_set_actions(struct wl_data_source *s, uint32_t actions)
{
    wl_data_source_set_actions(s, actions); /* since v3, drag sources only */
}

void wsi_data_source_destroy(struct wl_data_source *s)
{
    wl_data_source_destroy(s);
}

int wsi_data_device_add_listener(struct wl_data_device *d,
    const struct wl_data_device_listener *l, void *data)
{
    return wl_data_device_add_listener(d, l, data);
}

void wsi_data_device_set_selection(struct wl_data_device *d,
    struct wl_data_source *src, uint32_t serial)
{
    wl_data_device_set_selection(d, src, serial);
}

void wsi_data_device_start_drag(struct wl_data_device *d,
    struct wl_data_source *src, struct wl_surface *origin,
    struct wl_surface *icon, uint32_t serial)
{
    wl_data_device_start_drag(d, src, origin, icon, serial);
}

void wsi_data_device_release(struct wl_data_device *d)
{
    wl_data_device_release(d); /* since v2 */
}

int wsi_data_offer_add_listener(struct wl_data_offer *o,
    const struct wl_data_offer_listener *l, void *data)
{
    return wl_data_offer_add_listener(o, l, data);
}

void wsi_data_offer_accept(struct wl_data_offer *o, uint32_t serial, const char *mime)
{
    wl_data_offer_accept(o, serial, mime);
}

void wsi_data_offer_receive(struct wl_data_offer *o, const char *mime, int32_t fd)
{
    wl_data_offer_receive(o, mime, fd);
}

void wsi_data_offer_set_actions(struct wl_data_offer *o, uint32_t actions, uint32_t preferred)
{
    wl_data_offer_set_actions(o, actions, preferred); /* since v3 */
}

void wsi_data_offer_finish(struct wl_data_offer *o)
{
    wl_data_offer_finish(o); /* since v3, dnd offers only */
}

void wsi_data_offer_destroy(struct wl_data_offer *o)
{
