// ImportC shim for the F11 scroll demo (see ./app.d and ../../scaffold.md).
// Same three jobs as the scaffold's shim — one translation unit for the
// libwayland ABI + the scanner-generated xdg-shell glue (see ./generate.sh),
// POSIX shm pieces, and `wsi_*` re-exports of every `static inline` request
// helper ImportC cannot call. The scroll machinery itself needs nothing
// beyond core wl_pointer.
...
void wsi_vptr_axis_source(struct wl_proxy *p, uint32_t source)
{
    wl_proxy_marshal_flags(p, 5, NULL, wl_proxy_get_version(p), 0, source);
}

void wsi_vptr_axis_discrete(struct wl_proxy *p, uint32_t time, uint32_t axis,
    wl_fixed_t value, int32_t discrete)
{
    wl_proxy_marshal_flags(p, 7, NULL, wl_proxy_get_version(p), 0, time, axis, value, discrete);
}
