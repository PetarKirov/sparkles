...
| [F16 clipboard + DnD][f16]        | [A] clipboard + DnD share one `data_source`/offer pipe-fd machinery; `set_selection` is DEAD without a real input serial (silently ignored); DnD action compositor-resolved ([findings][wl-f16])               | [A] clipboard + XDND are ONE selection mechanism; INCR proven both directions; full XDND v5 source+target by hand; delayed rendering is the only mode ([findings][x11-f16])                                             |                                                                                                                                                                                                                        |                                                                                                                                                                                                                                       |
