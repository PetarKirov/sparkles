...
| ------------------ | -------------- | ---------------------- | ------------------------------------------------------------- | --------- | -------------------------------- | --------------------------------------- | :------------------------: | -------------------------- |
| **npm**            | JS/TS          | JS/TS package manager  | Root-package: `workspaces` glob in `package.json`             | No        | Install only (`_cacache`)        | Plain semver; symlink if satisfied      |    [✅](./npm/sample/)     | [npm][npm]                 |
| **Yarn Berry**     | JS/TS          | JS/TS package manager  | Root-package: `workspaces` glob; `workspace:` protocol        | Yes       | Install only (`.yarn/cache`)     | `workspace:` protocol + `portal:`       | [✅](./yarn-berry/sample/) | [Yarn Berry][yarn-berry]   |
| **pnpm**           | JS/TS          | JS/TS package manager  | Virtual root: `pnpm-workspace.yaml` globs                     | Yes (pkg) | Install only (CAS store)         | `workspace:` protocol + `catalog:`      |    [✅](./pnpm/sample/)    | [pnpm][pnpm]               |
...
| **uv**             | Python (Rust)  | Python package manager | Cargo-inspired `[tool.uv.workspace]`; one `uv.lock`, one venv | No        | Local CAS (`~/.cache/uv`)        | `[tool.uv.sources] {workspace = true}`  |     [✅](./uv/sample/)     | [uv][uv]                   |
...
| **Cargo**          | Rust           | Language build system  | Root `[workspace]`; root-package or virtual; one `Cargo.lock` | Yes       | Local (`target/` + fingerprints) | `path` + `[workspace.dependencies]`     |   [✅](./cargo/sample/)    | [Cargo][cargo]             |
| **Go (`go.work`)** | Go             | Language build system  | Virtual root `go.work` `use`-list of local modules (MVS)      | No        | Local CAS (`$GOCACHE`)           | Implicit: every `use`d module is local  |  [✅](./go-work/sample/)   | [Go `go.work`][go-work]    |
...
| **Nx**             | JS/TS          | JS/TS orchestrator     | Inherited from package manager + inference plugins            | Yes       | Local + remote (Nx Cloud)        | Delegated to package-manager symlinks   |     [✅](./nx/sample/)     | [Nx][nx]                   |
| **Turborepo**      | JS/TS          | JS/TS orchestrator     | Inherited; adds only `turbo.json` task layer                  | Yes       | Local + HTTP remote cache        | Package-manager `workspace:`/`*`        | [✅](./turborepo/sample/)  | [Turborepo][turborepo]     |
...
| **Bazel**          | Polyglot       | Polyglot orchestrator  | Single repo at `MODULE.bazel`; `//`-label packages            | Yes       | REAPI (cache + execution)        | `//path:target` label                   |   [✅](./bazel/sample/)    | [Bazel][bazel]             |
...
| **moon**           | Polyglot       | Polyglot orchestrator  | Root `.moon/`; `moon.yml` id→path map / globs                 | Yes       | Local + REAPI v2 cache           | Host `workspace:*` + `dependsOn`        |    [✅](./moon/sample/)    | [moon][moon]               |
