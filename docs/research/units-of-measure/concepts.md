...
system of quantities and a given set of base units" — `cm/s` is coherent in CGS, not in
the SI), and it is fragile: prefixes destroy it ("when prefixes are used with SI units,
...
`Rational{Int}` powers — the [comparison][comparison]'s "exact factors stay exact"
consensus), and minimal systems like [F#][fsharp] ship no metrology at all — every
...
- **The algebra is stated in the spec, not implied.** UCUM defines equality `=` and
  commensurability `~` as equivalence relations; units are the `=`-classes and "(`U`,
  ·) is an Abelian group" (§18); **dimensions are the `~`-classes** (§19); any system
...
- **Kind is mandatory; dimension hangs off the kind.** A `qudt:Unit` must carry at
  least one `qudt:hasQuantityKind` (SHACL `minCardinality 1`), and a
