...
> [!NOTE]
> **Scope.** Sixteen systems, surveyed across nine deep-dives. Deferred, noted
> not omitted: Buck2 and Pants `glob()` (both follow Bazel's include/exclude
> shape), Sapling's fork of Mercurial's fileset language, and `.dockerignore` (a
> `.gitignore`-shaped dialect with divergences of its own). Out of
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------- |
| Set algebra          | [jj][jj], [jj revsets][revsets], [Mercurial][hg], [Bazel query][bazel], [nixpkgs][nix]                                     | yes            |
| Ordered rules        | [`.gitignore`, rsync, npm, CODEOWNERS][ordered], [Bazel `glob()`][bazel], [git pathspec][pathspec], [`fd`, ripgrep][shell] | **no**         |
...
| A tree that must be walked     | [`.gitignore`][ordered], [pathspec][pathspec], [nixpkgs][nix], [`fd`/rg][shell], [`glob()`][bazel] | **no**                 |

### By anchoring rule

| Rule                 | Subjects                                                                             |
| -------------------- | ------------------------------------------------------------------------------------ |
| Root/cwd anchored    | [jj][jj], [Mercurial `glob:`][hg], [git pathspec][pathspec], [Bazel `glob()`][bazel] |
| Float unless slashed | [`.gitignore`, `.hgignore`, `.npmignore`][ordered], [ripgrep][shell]                 |
| Suffix anchored      | [rsync][ordered]                                                                     |
| Base name by default | [`fd`][shell]                                                                        |
...
- **Implementing the planner** — [concepts] § lattice, [pathspec] § evaluation,
  [ordered] § pruning, [hg] § evaluation (the cost model).
...
fff [`3a0ce85c`][fff-rev], rsync [`8b8de523`][rsync-rev], Mercurial tag `7.1`,
