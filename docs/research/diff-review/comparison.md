...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| `DVM1`–`DVM2` line diff + pairing  | [VS Code](./vscode.md) two-phase Myers/DP + boundary snapping; [delta](./delta.md) distance-gated pairing                      | new D code; adopt the chunk-stream model + similarity pairing from day one                                    |
...
| `DVM4` word refinement             | [Gerrit](./gerrit.md) hygiene passes; [diffview-plus.nvim](./diffview-plus-nvim.md) guards; [Meld](./meld.md) equal-run filter | port the guard recipes verbatim; subword tokens with hex-run coalescing                                       |
