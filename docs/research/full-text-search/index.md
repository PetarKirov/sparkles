...
[concepts § engine classes](./concepts.md#engine-classes) → `std-regex` →
`rust-regex` → `recommendations`.

**"I want to know whether we should build an index."**
[concepts § precomputation](./concepts.md#precomputation) → `trigram-indexes/` →
