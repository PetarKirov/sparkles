...
1. [Introduction — the four sub-problems of a span model](#1-introduction)
2. [The five representation strategies](#2-the-five-representation-strategies)
3. [Reference model — the HTML slot grid](#3-reference-model--the-html-slot-grid)
4. [Reference model — `tbl` key-letter sentinels](#4-reference-model--tbl-key-letter-sentinels)
5. [Terminal table renderers — deep dives](#5-terminal-table-renderers--deep-dives)
6. [TUI frameworks — span as a non-feature](#6-tui-frameworks--span-as-a-non-feature)
7. [Document & markup span syntaxes](#7-document--markup-span-syntaxes)
8. [GUI grid layouts — the sparse absolute-placement model](#8-gui-grid-layouts--the-sparse-absolute-placement-model)
9. [Comparative analysis](#9-comparative-analysis)
10. [Analysis through Sean Parent's principles](#10-analysis-through-sean-parents-principles)
11. [Design principles for a Sparkles span-capable table](#11-design-principles-for-a-sparkles-span-capable-table)
...
libraries surveyed here differ primarily in *which* sub-problems they solve and *how*:

- **Representation** — how a span is *encoded* in the data model: an attribute on the
  anchor cell, a separate descriptor list, a continuation sentinel, or implicit from content.
- **Occlusion** — which grid positions a span *covers*, and how those covered positions are
...
fully-specified models therefore come from *outside* the terminal world — the HTML table
...
- **Dense-stream (S1/S2/S3) vs. sparse-placement (S5).** The document/markup world is
  *dense*: a row is an ordered stream and a span causes following positions to be *omitted*
  (S1) or marked (S3). The GUI world is *sparse*: every cell names its own `(row, col)`
...
- **Overlap is the universally under-specified corner.** Only HTML defines overlap as a
  named error with deterministic behaviour; CSS Grid, GTK, and Android explicitly *permit*
  overlap and define paint order or call it "undefined"; Qt, Swing, Tk, and every terminal
  library leave it silent. A clean Sparkles model should make overlap a *defined* outcome
...
on the grid) and the **cell** (a content object that *covers* a rectangle of slots).
...
of covered slots is *derived*, never stored. `colspan`/`rowspan` are the public spelling of
`width`/`height`:
...
end of the row group" (a *downward-growing* cell), the only deferred/unbounded extent in any
system surveyed.
...
This is the algorithmic heart of strategy **S1**: covered cells are *omitted* from the row
stream, and the renderer recovers their positions by advancing past occupied slots. Depth
(here, column position) is *derived from the cursor*, not stored on the cell — directly
...
content. Real entries use `L`/`R`/`C`/`N`; spans use two dedicated *continuation* letters that
...
spans unambiguous *by construction*. The cost is verbosity (every covered position must be
...
*absence* of an interior `|` or `-` border, and the merged cell is the maximal rectangle in the
box-drawing art. Notably, reST's *simple* tables support column spans but **not** row spans —
...
each picks a *different* one of the five strategies — making this the most instructive cluster.
...
```javascript
table.push(
  [{ content: 'hello', colSpan: 2 }, { rowSpan: 3, content: 'hi' }],
...
HTML S1 model. A `rowSpan` therefore makes the *following* rows supply one fewer cell, so the
...
Here `data` must stay a **complete rectangular grid**, and spans are declared in a *separate*
...
The data stays a dense `[][]string`; the renderer merges *adjacent cells holding identical
strings* — vertically (a repeated column value across consecutive rows becomes a visual
...
(`SetAutoMergeCellsByColumnIndex`). S4 is wonderful for *reports* (collapsing a repeated
category column) and useless for *layout* (you cannot merge two cells that happen to share a
...
*column* spanning — correcting the older catalog claim that Ratatui has no span at all.
...
*shape* of the "fake it" escape hatch that every no-span library falls back to.

FTXUI's `Table` is built from a 2-D `std::vector<std::vector<Element>>` and exposes only
*selection + decoration*, never merging:
...
*individual* interior junction for a spanned cell, so true merges are not cleanly achievable.
...
### Dear ImGui / ImTui (C++) — horizontal *background* spanning only
...
explicit that even the *background* can only span horizontally:
...
Source: [`Brick.Widgets.Table`][brick-table-docs]. Brick's `Table` *refuses* non-rectangular
...
That is *width redistribution*, not merging. Real spanning in tview exists only in the separate
`Grid` *layout* primitive, whose `AddItem(p, row, column, rowSpan, colSpan, …)` is a textbook
**S5** sparse-placement signature — but `Grid` lays out arbitrary primitives, it is not the
cell-grid `Table`. The split is itself a finding: tview keeps the *data table* dense and
span-free, and pushes spanning into a *layout* container.
...
LaTeX splits the two axes across two mechanisms with *opposite* representations:

- `\multicolumn{n}{align}{text}` **replaces** *n* column entries with one — covered columns are
  *omitted* from the row (fewer `&`). This is S1.
...
Overlap is entirely the author's responsibility — `\multirow` silently *overwrites* whatever is
...
with a *cell specifier*, distinguishing **span** (`+`) from **duplicate** (`*`):

- `2+|` — colspan 2; `.2+|` — rowspan 2; `2.2+|` — a 2×2 block (column factor before the dot,
  row factor after).
- `3*|` — *duplicate* into 3 independent cells (not a span).
...
This explicit *span-vs-duplicate* distinction is a useful API nuance: "repeat this value across
...
the excess is ignored"). Org-mode similarly offers only `/`-row *column groups* (vertical
...
dimensions are *inferred* from the maximum coordinate used. Overlap is where these systems
...
[CSS Grid][css-grid] is the richest reference because it cleanly separates the *two axes* a
...
Auto-placement *never creates* overlap (each step "increment[s] the column position … until
this item's grid area does not overlap any occupied grid cells"), but *explicit* placement may,
with paint order resolved by `z-index`/`order`. [Textual's][textual] `column-span` / `row-span`
CSS properties are the terminal-world adoption of exactly this model — applied to *widgets in a
grid container*, not to table cells.
...
diff) at the cost of redundant covered content. For a *library* API consumed by other code, S2's
shape-stability is the stronger default; for a *human-authored* table, S1 reads better.
...
column axis. The reason is rendering, not representation: a colspan only suppresses *vertical*
...
S5's `(row, col, rowSpan, colSpan)` is order-independent and naturally a *value* (a flat array
of placements) — which is why it dominates GUI toolkits and why CSS Grid can offer a
sparse-vs-dense *policy*. S1's "omit covered cells, recover by cursor" is more compact for dense
...
auto-merge serves *report compression* (collapse a repeated category), not *layout*. It should
...
*consequence* of the placement cursor's walk — there is no object that *is* the covered region.
...
The **HTML slot grid** is the ideal: it makes both the *slot grid* and the *cell* explicit
...
cell data, separable from rendering. A Sparkles design should likewise express *resolve coverage*
(anchor + extent → covered slots, with overlap detection) and *resolve junctions* (which interior
...
per-cell capabilities (alignment, vertical-alignment, style) should be *optional primitives*
...
`anchor (row, col) + extent (rowSpan, colSpan)` and coverage is *derived*, never stored
...
flow a spanning cell's min-width as a *constraint across its columns* (as cli-table3 and CSS
...
separate, optional *rendering* pass over a span-free grid, not the span model.
