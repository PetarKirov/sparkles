...
Follow the project's [functional/declarative guidelines][functional-guidelines]: the tree renderer writes to any output range, not just stdout. This enables rendering to `Buffer`, `appender!string`, or a terminal buffer.
...
Follow the project guidelines: tree traversal and rendering should work with `Buffer` and avoid GC allocation. The flat storage model (contiguous `Node[]`) is naturally `@nogc`-friendly — no recursive heap allocation needed during traversal.
