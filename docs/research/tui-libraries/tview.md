...
The proportional size algorithm (linear distribution of remaining space by weight) is simple enough to implement `@nogc` with a fixed-size scratch buffer for tracking item sizes. A `UniqueBuffer!(FlexItem, 16)` would handle typical layout trees without allocation.
