...
### Buffer Abstraction -> D Output Ranges and @nogc Buffers
...
struct Buffer {
    Rect area;
    UniqueBuffer!(Cell, 4096) content;  // stack-allocated for typical terminal sizes
...
The `Buffer` from sparkles/core-cli avoids GC allocation for buffers that fit in the
