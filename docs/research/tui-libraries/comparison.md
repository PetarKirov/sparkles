...
**D suitability:** Excellent. Widgets as stack-allocated `struct` values, consumed within a single `draw` call, align perfectly with `@nogc` and output range patterns. Ratatui's `Buffer`-based approach maps directly to `UniqueBuffer!(Cell, N)`. libvaxis's explicit allocator passing maps to D's `@nogc` attribute, which provides the same guarantee (no hidden allocation) enforced at compile time rather than by convention.
...
**D suitability:** The plane compositor model maps well to D. Planes as `@nogc` structs with RAII cleanup via `~this()`, channel-based coloring via bitwise operations, and compositor output to `Buffer` -- all align naturally.
...
**D alignment:** Excellent. FTXUI's architecture maps remarkably well to D. The pipe operator IS D's UFCS -- `text("hello").bold.color(Color.blue).border` -- with zero operator overloading needed. The dual dom/component layers map to `@nogc` pure element types + DbI-detected component capabilities. FTXUI's `FlexboxConfig` struct maps to named-argument D structs with CTFE validation. The key improvement D offers: elements as `@nogc` value-type structs in `Buffer` instead of `shared_ptr` heap nodes.
...
**@nogc constraint solving:** A Cassowary solver operating on `Buffer`-backed vectors could solve layout constraints without GC allocation, making the entire render path `@nogc`.
...
**FTXUI's value-based elements** create `shared_ptr<Node>` for every element every frame. In D, elements as `@nogc` struct values stored in `Buffer` eliminate this heap allocation entirely. For small-to-medium UIs (the common case in CLI tools), the entire Element tree fits in a `Buffer` with no heap allocation at all.
...
**Nottui's incremental computation** avoids the allocation problem differently: the reactive DAG is allocated once and reused. Updates touch only damaged nodes, so neither allocation nor computation scales with total UI size. The `Lwd_table` reactive collection tracks insertions/deletions incrementally. For D, a pre-allocated `DependencyGraph` with `Buffer`-backed node arrays achieves the same pattern `@nogc`.
...
| **Ratatui-style cell buffer**         | **High**          | `UniqueBuffer!(Cell, N)` for typical screens                          |
...
| **Bubble Tea-style string render**    | **Moderate**      | `UniqueBuffer!(char, N)` instead of heap string                       |
...
3. **Ratatui-style cell buffer** -- Near-zero allocation with `Buffer`, cell-level diff, immediate-mode simplicity. Best balance of performance and ergonomics.
4. **FTXUI-style functional DOM (in D)** -- Elements as `@nogc` structs in `Buffer` instead of `shared_ptr`. Combines compositional elegance with zero allocation.
...
2. **FTXUI's functional DOM composition translates beautifully to D UFCS.** Where FTXUI writes `text("hello") | bold | color(Color::Red) | border`, D writes `text("hello").bold.color(Color.red).border`. No special pipe operator needed -- UFCS is built-in, works at compile time, and composes with any free function. Elements as `@nogc` struct values in `Buffer` instead of `shared_ptr` nodes eliminate FTXUI's per-node heap allocation. This is arguably the single most important lesson: FTXUI proved the functional DOM pattern works for terminals; D's UFCS makes it even more natural.
...
struct Buffer {
    Rect area;
    Cell[] cells;  // or UniqueBuffer!(Cell, 80 * 24) for typical terminals
...
- `Cell` struct: grapheme (`Buffer`-based) + style + fg/bg color
...
This phase builds on the existing `term_style` module (extending it) and `Buffer` (for cell graphemes and buffer storage).
