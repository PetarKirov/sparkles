...
- **Flat array representation**: The `Vec<`[`TreeLine`][broot-tree-line]`>` model maps directly to a D `TreeLine[]` or `UniqueBuffer!(TreeLine, N)`. This is the simplest correct representation for a scrollable, selectable tree view.
