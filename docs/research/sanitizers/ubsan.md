...
**Nor is there a side door through ImportC.** LDC's `-Xcc=<flag>` reaches exactly two
places: the C preprocessor it runs over an ImportC source (`cc -E`), and the C compiler it
uses as the link driver. The preprocessed C body is then parsed by the DMD C frontend and
code-generated by LDC's own IR backend — the same backend that has no UBSan pass. So
`-Xcc=-fsanitize=undefined` instruments nothing and `-Xcc=-fstack-clash-protection`
protects no frame; the only effect is that the link driver pulls `libubsan` into a binary
that never calls it. Measured on LDC 1.42.0 with a C file holding a signed `a + b` and a
variable `p[i] << i`: `ldc2 -c -output-ll -Xcc=-fsanitize=undefined` emits **0**
`__ubsan_handle_*` call sites, against **6** from the host clang on the same file with
the same flag. `[hw-verified: aarch64-darwin]`. Every `.c` dub compiles in this repository
(the `libs/wsi` XCB/Wayland bridges, the ghostty and tree-sitter ImportC shims) takes this
path, and the C libraries themselves are prebuilt by nix and never recompiled — which is
why `ci --test-sanitize` carries ASan and LDC's stack-overflow sanitizer only.
