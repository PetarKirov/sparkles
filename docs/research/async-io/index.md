...
| **GCD / libdispatch** | C          | Reactor (kernel-delivered upcalls)         | kqueue (`kevent_qos`/`kevent_id`); epoll + `/proc` monitor off Darwin              | Hierarchical serial/concurrent queues; kernel-owned pool      | OS concurrency service       | [gcd/][gcd]                                 |
...
| [GCD][gcd]               |    ✅ (kqueue only)     |       —       |           —           |  ✅ (kernel workqueue)  | none — Darwin has no completion interface; `dispatch_io` is `pread` on threads |
...
| ------------------------- | ---------------------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Reactor (readiness)**   | "fd is _ready_ — you may syscall without blocking now"                             | epoll, kqueue, event ports, poll/select | [Tokio][tokio] (default), [Go][go], [Python][python] (Unix), [.NET][dotnet] (today), Java NIO, [GCD][gcd], [Cats Effect][cats-effect], [ZIO][zio], [Effect-TS][effect-ts] |
...
| ------------------------------------- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Callbacks**                         | A registered continuation function invoked on the event | [libuv][libuv], [GCD][gcd] (a handler on a queue), [Boost.Asio][boost-asio] (callback token), asyncio's `add_reader`/`add_writer` floor ([Python][python])  |
...
| **OS service**          | The platform's own concurrency substrate, shared process-wide    | [GCD / libdispatch][gcd] on Darwin (kernel workqueue + kqworkloops)                                                                                                                  |
...
| -------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 2009           | —                                                                                             | **Grand Central Dispatch** ships in Mac OS X 10.6 and is open-sourced — a kernel-integrated queue/thread service        |
...
| 2018           | —                                                                                             | **`dispatch_workloop`** made public in macOS 10.14 — kernel-ordered, priority-evaluating queues ([GCD][gcd])            |
...
| [GCD][gcd]               | macOS's own concurrency service: queues + kqueue sources, with the thread pool in the kernel.       |
