...
| **Owned-buffer (move/`scope`) API for I/O**                                          | Completion semantics _force_ it — a borrowed slice is unsound across submission→CQE. Mirror Monoio's `IoBuf`/BufResult with D move semantics.                                                                                              | Less ergonomic than borrowed `read(buf[])`; users must thread buffers through; needs a clean `Buffer`/pool story.                         |
...
| Structured-concurrency scopes / timers             | `@nogc nothrow`; allocation via `Buffer`/pools                       |
...
{
    auto buf = UniqueBuffer!(ubyte, 4096)(); // or a pooled / registered buffer
    for (;;)
    {
...
`Buffer`/pooled buffers (the [agent guidelines][agents]).
...
- For the common case, back buffers with `Buffer` (the repo's `@nogc` dynamic buffer) or a
...
| Owned buffers vs. ergonomics      | Sound, but less convenient than borrowed slices                                             | Owned is non-negotiable for a proactor; invest in `Buffer`/pool ergonomics + provided rings to soften it.                                               |
