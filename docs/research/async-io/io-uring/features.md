...
> **Scope and ground truth.** Type names, struct fields, and flag spellings in this document are quoted from a Linux tree at **v7.3-rc4** (`93f51579e7df248780214094418f205253383cc5`, 2026-09-20). The previous pass was **v7.1-rc6**. `include/uapi/linux/io_uring.h` is identical across that range — no new `IORING_OP_*`, `IORING_SETUP_*`, `IORING_FEAT_*`, or `IORING_REGISTER_*` value. The UAPI that moved is the split headers `include/uapi/linux/io_uring/{zcrx,query,bpf_filter}.h`, plus the behavior of several existing opcodes (see [Timeline](./timeline.md) from 7.1 on). The liburing tree cited below is `78dce99b` (2026-09-11, `liburing-2.15` plus 35 commits; the spec file still says version 2.15). Its copied UAPI stops short of the 7.2 ZCRX-event, query, and BPF-connect structs — see [the chronology's liburing section](./timeline.md#liburing-companion). Version markers ("since Linux 6.x") are cross-checked against liburing man pages and Jens Axboe's per-release update notes — see [Sources](#sources). Where a feature is brand new (kernel 6.16 / 7.x), it is flagged as such; an event loop targeting a stable distro kernel must probe (`IORING_REGISTER_PROBE` / `IORING_REGISTER_QUERY`) rather than assume.
...
[liburing/examples/proxy.c]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/examples/proxy.c
[liburing/examples/send-zerocopy.c]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/examples/send-zerocopy.c
[liburing/examples/zcrx.c]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/examples/zcrx.c
[liburing/examples/napi-busy-poll-server.c]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/examples/napi-busy-poll-server.c
[liburing/man/io_uring_multishot.7]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/man/io_uring_multishot.7
[liburing/man/io_uring_linked_requests.7]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/man/io_uring_linked_requests.7
[liburing/man/io_uring_cancelation.7]: https://github.com/axboe/liburing/blob/78dce99b660fd1caaf70ef6573a88bae9bbf6476/man/io_uring_cancelation.7
