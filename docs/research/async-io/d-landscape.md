...
Sparkles (per the repo guidelines) targets **maximum safety attributes**, `@nogc`/`nothrow` hot paths, `-preview=in` and `-preview=dip1000` scope semantics, and `Buffer`-style allocation avoidance. That constraint set rules the existing frameworks out as foundations:
