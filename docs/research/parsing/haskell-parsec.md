...
- **Scannerless by default.** The token type is the stream element (`Char`, a byte, or a user `Token s`); there is no separate lexer phase. A lexer _layer_ is optional and itself built from combinators (`Text.Megaparsec.Char.Lexer`'s `lexeme`/`space`/`symbol`). This is the same scannerless stance as [PEG tools][theory-peg], and unlike the two-phase [Bison/flex][bison] or [ANTLR][antlr] pipelines and [tree-sitter][tree-sitter]'s generated context-aware lexer.
