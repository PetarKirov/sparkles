...
7. **How do tools turn a parse into colors?** The two syntax-highlighting models —
   the line-local [TextMate scope machine][sh] ([syntect], [Shiki][shiki], consumed
   by [bat]) vs [CST queries][ts-highlight] over a recovered tree — the theme
   formats, and the pipeline a `bat`-style tool builds around an engine. See the
   [syntax-highlighting synthesis][sh].

> [!NOTE]
> **Scope: wave 1 (foundation + flagship) + three wave-2 clusters + the wave-4
...
> the [D landscape][d-landscape]; **wave 4 adds the syntax-highlighting cluster**
> ([syntect], [bat], [tree-sitter-highlight][ts-highlight], [Shiki][shiki] + the
> [synthesis][sh]), the evidence base for the planned `sparkles:syntax` library. Still
> deferred: **generators** (LALRPOP, Lark/PLY, Peggy, Ragel/re2c) and a few second-tier
> Scala/JVM combinators (FastParse, parsley, cats-parse). Rows a future deep-dive will
> add are noted, not silently omitted.
...
> [Angstrom][angstrom], [FParsec][fparsec]). Wave 4 adds **Syntax highlighting**
> ([syntect], [bat], [tree-sitter-highlight][ts-highlight], [Shiki][shiki]; synthesized
> in [syntax-highlighting][sh]). Still deferred: **Generators** — LALRPOP
...
| **Stateful line re-lexing (TextMate scopes)** | Interpret a regex+scope grammar one line at a time, carrying a context/scope stack between lines — approximate, tree-free, never fails | [syntax-highlighting][sh]                   | [syntect], [Shiki][shiki], consumed by [bat]; the CST-query counterpart is [tree-sitter-highlight][ts-highlight]                                                                |
...
| **Runtime-interpreted grammar data**    | Declarative pattern/query files loaded and interpreted at run time — no codegen: `.sublime-syntax`, `.tmLanguage.json`, `.scm` queries | [syntect], [Shiki][shiki], [tree-sitter-highlight][ts-highlight] (queries over a generated grammar); [bat] ships them pre-serialized                                 |
...
| **Degrade-gracefully (highlighting)** | Never reject input; worst case is unstyled or mis-scoped text, with guards bounding pathological cost | [syntect], [bat] (16 KiB line cutoff), [Shiki][shiki] (length/time budgets), [tree-sitter-highlight][ts-highlight] (`ERROR` spans + cancellation)          |
...
| 2017–2018   | **Jourdan & Pottier 2017** — verified C11 LR parser (TOPLAS); **_Build Systems à la Carte_** (Mokhov et al., ICFP 2018)              | **tree-sitter** (Brunsfeld/GitHub, 2018) — [incremental GLR][tree-sitter]; **[rust-analyzer]** + **[salsa]** (2018); **[bat]** v0.1.0 (sharkdp, Apr 2018); **[Shiki][shiki]** (Pine Wu, Oct 2018)                                                                                                                              |
...
- **"I want the syntax-highlighting story."** [syntax-highlighting][sh] (the two
  models + vocabulary) → [syntect] (the TextMate engine) → [bat] (the CLI pipeline
  around it) → [tree-sitter-highlight][ts-highlight] (the precise counterpart, read
  against [tree-sitter]) → [shiki] (the web/HTML side) → the
