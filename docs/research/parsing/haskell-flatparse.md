...
flatparse is the survey's **existence proof for a `@nogc` combinator API**. Every other combinator library here allocates on the parse path; flatparse shows that with an unboxed result and a raw-buffer input, a combinator parser returning `()` can run with **zero heap allocation** — precisely the target for a D combinator written against `SharedBuffer`/`@nogc` text primitives rather than the GC. Three mappings are direct:
