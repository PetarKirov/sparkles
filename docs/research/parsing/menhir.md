...
| `--GLR`       | Generalized LR back-end (new in `20260122`) for ambiguous grammars                         |
...
**Formalism.** Menhir is a [bottom-up, shift-reduce LR parser generator](./theory/bottom-up.md). It builds a deterministic pushdown automaton whose action (shift or reduce) is chosen from the current state and **one** token of lookahead. The accepted class is **LR(1)** by default — the largest class a deterministic single-lookahead bottom-up parser can handle — narrowing to LALR(1) under `--lalr` or widening to the full canonical LR(1) under `--canonical`. As of `20260122`, an optional `--GLR` back-end lifts the determinism requirement entirely, splitting the parse stack to explore ambiguous alternatives in parallel (the [general-parsing](./theory/general-parsing.md) regime that [tree-sitter](./tree-sitter.md) also inhabits).
