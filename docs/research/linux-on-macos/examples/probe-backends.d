...
/// One backend's raw report. A missing executable is the normal case on a
/// CI runner (no `determinate-nixd`, no `container`), so it is a SKIP line,
/// not an exception out of `main`.
void probe(string name, string[] cmd)
{
    ...
    {
        writeln("== ", name);
        writeln("SKIP: ", cmd[0], " is not installed (", e.msg, ")");
    }
}
