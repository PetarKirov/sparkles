...
| [`examples/leaf-dispatch.d`](./examples/leaf-dispatch.d)                     | D2/D4: a closed `static if` ladder to `LeafKind`, a UDA vocabulary, `@ShowIf("expr")` as a typed `@safe` predicate, the opaque-value escape — and negative probes: a mistyped `@Editor` symbol, an `@Editor` on an opaque leaf, or a bad `@ShowIf` member is a build error                                         |
| [`examples/path-addressing.d`](./examples/path-addressing.d)                 | D3: `a.b[2].c` resolved at compile time (a `ref`-returning access) and at run time (a generated walk), proved equal by differential test — index paths shown positional, then fixed by element-provided `propElementKey` identity (`[#7]`, duplicates refused) plus a quoted segment form for non-identifier names |
