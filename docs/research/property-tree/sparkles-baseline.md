...
| Type → rows (reflection)              | runtime in seven subjects; **compile-time** in the [derive crates][derive]                                                             | nothing                                                      | **missing** — and compile-time here, with a hard recursion limit                                                   |
...
| Validation + error display            | [WinForms][winforms] modal; [VS Code][vscode] **inline per row**; [rjsf][rjsf] path-addressed errors                                   | nothing                                                      | **missing**                                                                                                        |
| Collections editing                   | [Unreal][unreal] `AsArray`; [bevy][bevy] list ops; [Godot][godot] paged; [rjsf][rjsf] **stable row keys**                              | table core, read-only                                        | **missing**                                                                                                        |
| Variant / type picker                 | [bevy][bevy] constructable check; [derive crates][derive] generate the construction; [rjsf][rjsf] **migrates data**                    | nothing; `SumType` assignment is `@system`                   | **missing**, with a D-specific safety wrinkle                                                                      |
| Multi-object editing                  | [WinForms][winforms] merged descriptors; [Unity][unity] ambient `showMixedValue`; [Unreal][unreal] per-object values                   | nothing                                                      | **missing**                                                                                                        |
| Conditional visibility                | attributes ([WinForms][winforms]) / usage flags ([Godot][godot]) / restrictions ([Unreal][unreal]) / schema `if`-`then` ([rjsf][rjsf]) | nothing                                                      | **missing**                                                                                                        |
| Element identity in a collection      | [rjsf][rjsf] mints a synthetic row key; everyone else uses the index                                                                   | nothing                                                      | **missing** — and the corpus says the index is not enough                                                          |
| Provenance (default / scope / locked) | [VS Code][vscode] `isConfigured`, `defaultValueSource`, `hasPolicyValue`                                                               | nothing                                                      | **missing** — needed the moment this edits config rather than a value                                              |
| Bounding a hostile subject            | [DevTools][devtools] fetch policy; [Godot][godot] pagination                                                                           | `viewSlice` windows rows already materialised                | **partial** — no policy before materialisation                                                                     |
| Refusing to descend, with an escape   | [VS Code][vscode] `Complex` → edit the JSON                                                                                            | nothing                                                      | **missing** — the obvious answer for the read-only HTML target                                                     |
| An erasure boundary in the child walk | [derive crates][derive] `&mut dyn`                                                                                                     | nothing                                                      | **missing** — the escape from our compile-time recursion limit ([`erased-descent.d`](./examples/erased-descent.d)) |
...
- **The recursion limit is ours alone.** Every surveyed subject descends at runtime, so a
  self-referential type costs them a click; a D CTFE walk does not build at all
  ([`reflect-descent.d`](./examples/reflect-descent.d)). The corpus's escape — an erasure
  boundary in the child walk ([derive crates][derive]) — is measured for D in
  [`erased-descent.d`](./examples/erased-descent.d): it works, at one delegate per open node.
