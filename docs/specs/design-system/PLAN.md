...
- **Checked revision:** `feat/design-system-query-battery`, on top of PR #522
  (octants, merged).
...
- **Done (after M3):** `ui-gallery` switches its profile live (`}`/`{`),
  ...
  through the live parser (`O5`); decoders that drop late replies.
...
- **Next executable actions:** (1) negotiate bracketed paste and focus
  reporting in `sparkles:tui`, so their answers can be applied — and drop the
  focus/paste claim `PosixEvents` makes without them; (2) a consumer that
  degrades visibly per probed row (M7's gate); (3) Windows Terminal and the
  Linux console need a person at the machine; (4) M4, once the owner's design
