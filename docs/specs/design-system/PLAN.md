...
| **M1**    | Token vocabulary in code: `Slot` regrouped into tiers with the `TOK3` roles added, `resolve(slot, states)`, `Palette` metrics renamed (`TOK8`), every component declares `slots` with `O3` tests                                                            | `TOK1`, `TOK3`–`TOK8`, `TOK10`                 | `O3` over every `components/*.d`; every existing golden unchanged (the rest state is the old behaviour)                 | file format; CSS                                      | not started |
...
- **Checked revision:** the `feat/design-system` branch on top of `c9fe5bde7`.
- **Done:** M0 tree written; `tokens.d` draft.
- **Unverified:** everything in the [evidence ledger](./testing.md) marked so.
- **Blockers:** none for M1. M4 needs the brand design exercise (a session
  with the owner). M7's `textSizing` row is gated on the text-sizing proposal.
- **Next executable action:** M1 — add the `TOK3` roles to `Slot`, thread
  `StateSet` through `resolveVisual`, rename the `Palette` metrics.
