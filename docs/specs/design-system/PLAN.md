...
| **M2**    | `TargetCapabilities` declared by `ui-tui` (from `TermCaps`), `ui-raylib` and `html_semantic`; chrome half of `TGT5` closed; `ui-gallery --profile`; degradation report                                                                                      | `CAP1`, `CAP2`, `CAP4`–`CAP6`, `CAP8`          | `baseline` render of every gallery page as verified fences (`O1`); `CAP4` parity scenario                               | new probes (M6)                                       | not started                                                                                                                            |
...
- **Checked revision:** `feat/design-system-m1`, on top of `b6f1fc92d` (PR #505 merged).
- **Done (M0):** the tree, `tokens.d`, `TargetCapabilities` over `base`/`input`.
- **Done (M1, this branch):** `TOK3` roles with scheme defaults and derivation;
  `TOK8` metric renames with paths; `TOK6` declarations + `firstUndeclaredSlot`
  for the seven widget-building `ui` components; `TOK4`/`TOK5` state
  overlays on `Palette`, `resolveSlot`/`resolveVisual` and `Widget.states`.
- **Unverified / open in M1:** per-state attributes and metrics (D22);
  migrating the components that swap slots by hand (`actionBar` armed →
  `pressed`, scrollbar hover → `hover`) onto the overlays; `TOK6` for the
  twoslash/source-view/hue views; `TOK1`/`TOK10` are M5's (the alias graph
  needs the file format).
- **Blockers:** none. M4 needs the brand design exercise (after M1–M3, D23).
- **Next executable action:** M2 — `TargetCapabilities` declared by `ui-tui`
  (from `TermCaps`), `ui-raylib` and `html_semantic`; `ui-gallery --profile`;
