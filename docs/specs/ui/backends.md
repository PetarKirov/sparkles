...
| TGT12 | **Nested scissor stack intersection**: Concrete canvases supporting scissor clipping (`applyScissor`) must compute the geometric intersection (`intersectScissors`) between the active scissor rectangle and any pushed child scissor rectangle before issuing GPU scissor calls. Popping restores the previous intersection or clears the clip. A child container's scissor clip can never extend beyond an ancestor's clip bounds. | full                          | `libs/ui-raylib/src/sparkles/ui_raylib/raylib_canvas.d` `applyScissor`/`popScissor`                                                                                                                                                                                                                                                                     |
