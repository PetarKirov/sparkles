...
## Shipped targets (`TGT3`–`TGT6`, `TGT12`)
...
| TGT12 | **Nested clips intersect, and that is the canvas's obligation.** A canvas must clip to the intersection of every rect on its clip stack, not to the topmost one; popping restores the enclosing intersection. This is a requirement on the canvas because clips reach it from two sources: `buildDisplayList` emits clips already intersected by `childClipOf`, but an application also pushes clips by hand (`hue`'s base viewport, `diagram`'s board clip) and those are intersected against nothing. A canvas that replaces loses the ancestor on exactly those. `GridCanvas` satisfies this by testing each cell against the whole stack; `RaylibCanvas` folds the stack before issuing a GPU scissor. | full                          | `ui-raylib` `raylib_canvas.d` `effectiveClip`/`applyScissor`; `ui-tui` `grid_canvas.d` `inBounds`/`rowOutside`; `ui` `canvas.d` `pushClip`; `ui_raylib.raylib_canvas.effectiveClipIntersectsStack`                                                                                                                                                      |
...
| `libs/ui-raylib/src/`                        | `TGT5`, `TGT6`, `TGT9`, `TGT12` |
