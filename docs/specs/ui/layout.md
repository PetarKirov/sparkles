...
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | --------------------------------------------------- |
| LAY1 | Layout must turn a widget tree into one **`Frame` per node** — an absolute rectangle — via two passes, a bottom-up _measure_ and a top-down _place_, both `@safe pure nothrow`, in `O(n)` over the node count.                                                         | full (`4c66e6f3`) | `layout.d` `layout`/`measure`/`place`               |
...
| ID    | Requirement                                                                                                                                                                                                       | Status      | Traces to                               |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | --------------------------------------- |
| LAY11 | Nodes must support **tri-state visibility** — visible / hidden (occupies space) / collapsed (removed from flow) — so chrome can toggle without a layout jump. It maps to `visibility` and `display:none` on HTML. | not started | proposed `Widget.visibility`            |
| LAY12 | The laid-out tree must be **introspectable** — serializable to a readable form (nodes, kinds, slots, resolved rects) for debugging, via the existing pretty-printing machinery.                                   | not started | proposed `dumpTree`; `base.prettyprint` |
...
| L4        | Alignment, tri-state visibility, tree dump; delete `margin`            | not started | `LAY8`, `LAY11`, `LAY12` |
| L5        | Track sizer + wrapping strategy seam                                   | not started | `LAY9`, `LAY10`          |
...
| ---------------------------------------- | ------------------------------------------------------- |
| `libs/ui/src/sparkles/ui/layout.d`       | `LAY1`, `LAY2`, `LAY4`, `LAY6`–`LAY9`, `LAY11`, `LAY13` |
| `libs/ui/src/sparkles/ui/geometry.d`     | `LAY3`, `LAY5`, `LAY6`                                  |
| `libs/ui/src/sparkles/ui/canvas.d`       | `LAY5` (the `measure` primitive)                        |
| `libs/ui/src/sparkles/ui/display_list.d` | `LAY7` (scissor operations)                             |
