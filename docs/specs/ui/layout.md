...
| LAY14 | A plain text run must paint **within the frame layout gave it**. The display list cuts a `text` node's run, and each of its wrapped lines, to the frame's width in cells, so text that layout shrank in an overfull row, or an unbreakable word, never paints over a sibling, a border or a scrollbar: a canvas takes a run's advance from the text itself and cannot know the frame. `rich` spans are exempt, because a rich row is a content line whose overflow the enclosing clip scrolls (`LAY7`; hue's horizontal bar is sized from that overflow). | full              | `geometry.takeCells`; the `text` case in `display_list.emit`; `ui.display_list.textIsCutToTheWidthLayoutGaveIt`, `ui_gallery.gallery.aSidebarAtItsFloorKeepsItsCaptionsOffTheBorder` |
