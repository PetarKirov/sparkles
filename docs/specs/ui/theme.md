...
| THM7 | A theme that pins its page colors but carries no explicit palette must derive a **personalized** slot palette from its own `defaultFg`/`defaultBg` — chrome bands, panels, gutters, borders and mutes as the markdown design language's mixes, the chrome accent probed from the theme's own syntax rules (`function`/`markup.link`) — so every slot consumer (tab strips, header bars, scrollbars) **tracks the theme** instead of collapsing onto one shared grey per light/dark scheme. An explicit palette still wins outright. | full        | `Theme.effectivePalette` derivation; `ruleFgFor`                              |
