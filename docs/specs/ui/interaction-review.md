...
| ----- | --------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| IXR1  | Scrollbar geometry    | `scrollbarThumb` (STM2) — shared, property-tested          | uses it                                                                                     | RESOLVED: `OpKind.scrollbar` carries content units; `RaylibCanvas.scrollbar` resolves px geometry over STM2 | resolved — one formula; hover expansion is semantic `ScrollbarAnim.percent`                                        |
| IXR2  | Scrollbar interaction | `ScrollbarState` (STM9; grab-relative) inside `ScrollView` | RESOLVED: dock routes document/explorer/inspector bars; fences remain content-level clients | RESOLVED: one unconditional event drain reaches the dock; all pane bars and fence bars run `ScrollView`     | resolved — one machine, one click semantics (thumb grabs in place, track jumps the leading edge)                   |
| IXR3  | Scrollbar painting    | WGT10 → semantic scrollbar op                              | RESOLVED: both cell dispatchers use the shared fallback                                     | RESOLVED: `RaylibCanvas.scrollbar` owns the continuous px rail                                              | resolved — target-specific paint over one state/geometry model and one palette authority                           |
| IXR4  | Horizontal scrolling  | `ScrollbarState(ScrollAxis.horizontal)`                    | RESOLVED: dock-owned document/explorer/inspector axes                                       | RESOLVED: the same dock axes plus content-level fence bars                                                  | resolved — wide content is reachable on both interactive targets and in every pane                                 |
...
| IXR26 | Hover on a hoverless target  | `InputCapabilities.hover` (`IXB10`)          | hover served (`cellPointer`)          | PARTLY RESOLVED (`IXB10`): all four scrollbars expand by `ScrollbarState.expanded(caps)`; the twoslash popup still assumes hover | scrollbars resolved — the hover assumption is now a declaration a component reads, not one each host hardcodes                             |
| IXR27 | Action-bar hit test vs paint | `actionBar` + `PressState` (`IXB9`)          | n/a                                   | RESOLVED (`IXB9`): segments laid out once; hit rects come from `hoverTargets` over those frames                                  | resolved — the three defects are unrepresentable: non-visible ⇒ not hit-testable, ids arbitrate topmost-wins, the engine centres by column |
...
shipped with B-1 and was consolidated into the semantic scrollbar operation
(`ScrollbarState` + `WidgetKind.scrollbar` + `RaylibCanvas.scrollbar`, closing
