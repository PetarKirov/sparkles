...
| ----- | --------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| IXR1  | Scrollbar geometry    | `scrollbarThumb` (STM2) — shared, property-tested   | uses it                                                                           | uses its own `thumbGeometry` float variant + hover-expand widths                                                          | **two thumb formulas**; the GUI's animation state (`Scrollbar` struct ×2) is host-local                            |
| IXR2  | Scrollbar interaction | `ScrollState.pressedAt`/`draggedTo` (grab-relative) | `sbDragging`+`sbGrab` duplicated in `tui.d` AND `explorer.d`                      | its own pixel-space grab (`dragStartY`/`dragStartOffset`) ×2, track click **centers** instead of jumping the leading edge | **four grab implementations, two click semantics**                                                                 |
| IXR3  | Scrollbar painting    | none                                                | two hand-rolled cell painters (`tui.d` `paintScrollbar`, `explorer.d` paint tail) | two hand-rolled px painters with hover-expand animation                                                                   | **four painters**, no component                                                                                    |
