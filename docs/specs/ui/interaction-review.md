...
| IXR6  | Pointer capture       | `CaptureState` (STM11)                              | PARTLY RESOLVED: `workspace.d` runs the machine (press owns the drag)       | still the allow-list negation chain — converting it needs `RaylibEvents` wired (IXB7)                                              | one model now exists and the TUI runs it; the GUI's gates are spread across ~35 raw polls                          |
