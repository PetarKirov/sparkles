...
| ID    | Requirement                                                                                                                                                                                                                                                                                                    | Status | Traces to                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| EFX20 | A tier-0 transform must be **one D function serving both targets**: called per cell by `ui-tui`, and compiled to the GPU's shader from that same source. The registry stores both artifacts; it must never be the case that the GPU and terminal paths are two hand-written implementations that can disagree. | full   | `sparkles.ui.effect_shaders` — the four built-ins and the tier-1 warp, written once against `sparkles:shader`; `effect.d`'s `tier0Adapter` calls them per cell; `libs/ui/shaders/effects.d` wraps them in `@fragment` entry points; `shader-compile` derives `*.frag` into `libs/ui/generated/shaders/` at build time (the `gpu-effects` configuration)                                                                                                                                        |
| EFX25 | The GPU artifact must be **derived from the D source by the build, never kept beside it**: a build that uses it regenerates it whenever its inputs change, and a build that does not must not need the shader compiler at all.                                                                                 | full   | `sparkles:ui`'s default configuration carries no GPU half (`hasGpuEffects`; test `ui.effect.builtins.carryOnlyTheirCpuHalfWithoutGpuEffects`) and needs only the host compiler; its `gpu-effects` configuration, which `ui-raylib` selects, runs `shader-compile --if-stale` as a pre-generate step — a stamp of input hashes (`shaderCompile.stamp.freshUntilAnInputChanges`) — into the git-ignored `generated/shaders/`; Nix builds it once (`ui-shaders`) and hands apps a `prebuilt` copy |
...
speaks — proving each with glslang. `effect.d` string-imports the result, in the `gpu-effects` configuration only: the build generates it (a pre-generate step, no-op while fresh), so nothing generated is committed. The compiler ships as dlang.nix's `ldc-vulkan`, on the dev shell's `PATH` as `ldc2-vulkan`. The
