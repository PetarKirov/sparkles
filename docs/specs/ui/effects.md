...
| EFX28 | A shader module must be **diagnosable in the editor as the device build sees it**: analyzed with the dcompute LDC's runtime, versions and rules, a correct `@compute` module shows no errors, and one the device build would reject shows that build's error.                                                  | full   | `sparkles:dmd-lsp` target profiles — [`TGT1`–`TGT9`](../dmd-lsp/targets.md); `shader-units.json` is shared by the build and the analysis                                                                                                                                                                                         |
