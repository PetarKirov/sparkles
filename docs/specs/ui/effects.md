...
| IMG5 | `ui-tui` must draw images through a terminal image protocol where one is detected, and fall back to `IMG4` otherwise. Protocol detection is the terminal's, not the toolkit's.                                                         | partial | kitty: `sparkles.tui.probe` (the `images` query, fenced by DA1) and `sparkles.tui.images` (`KittyImages`: transmit once per generation, place when new or moved, delete when gone) under `grid_canvas.d`'s `image` arm, which records a placement where the image is wholly in view and outside an effect bracket; tests `tui.images.*`, `tui.probe.*`, `integration.pty.probeImages*`, `integration.pty.imagesGoOutInsideTheFrame`, `ui_tui.grid_canvas.kittyPlacement*`. Everywhere else the fallback is `GLY9`'s cell ladder, then `IMG4`. Sixel is not drawn yet. |
...
What remains: **sixel** needs an encoder (quantize to a palette, then
run-length the six-row bands). A sixel answer is declared as `none` until the
encoder exists, and the image takes the cell ladder. Below the protocol, an
