...
| EFX11 | Tier-1 and tier-2 effects require rendering the bracketed subtree to a texture. A GPU backend must scope that texture to the bracket, and nested brackets must not require more than one texture per nesting level. | none    | — the outstanding gate; see [Why the registry landed in gate 2](#why-the-registry-landed-in-gate-2)                                                       |
| EFX12 | Every registered effect must name the behaviour it degrades to on a canvas that cannot honour its tier — including "unaffected", stated rather than assumed.                                                        | partial | `Degradation`/`EffectRecord.degradation` exist and every built-in states one; nothing yet READS the field, because nothing yet fails to honour a built-in |
