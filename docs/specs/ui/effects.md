...
| ID    | Requirement                                                                                                                                                                                                         | Status | Traces to |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | --------- |
| EFX7  | Every effect must declare its tier. The tier is the contract: it says what the effect may read, and therefore which canvases can honour it.                                                                         | none   | —         |
| EFX8  | A **tier-0** effect is a pure colour transform: `(Point, RgbColor) -> RgbColor`, over the resolved `Ink` of each cell. It must be expressible with no texture and no neighbour reads.                               | none   | —         |
| EFX9  | `ui-tui` must honour tier-0 effects by applying the transform per cell as it composites, and must ignore tiers 1 and 2 per `EFX3`.                                                                                  | none   | —         |
| EFX10 | A tier-0 transform must be `@safe pure nothrow @nogc`. It runs per cell, and the constraint is also what keeps `EFX20` reachable.                                                                                   | none   | —         |
| EFX11 | Tier-1 and tier-2 effects require rendering the bracketed subtree to a texture. A GPU backend must scope that texture to the bracket, and nested brackets must not require more than one texture per nesting level. | none   | —         |
| EFX12 | Every registered effect must name the behaviour it degrades to on a canvas that cannot honour its tier — including "unaffected", stated rather than assumed.                                                        | none   | —         |
