...
| INP18 | The fold must carry a **horizontal wheel channel** (`wheelCellsX`): a horizontal axis (`WheelEvent.dx`, a trackpad's sideways swipe) accumulates directly, and a **Shift-modified vertical notch turns sideways** instead of scrolling — the universal spelling on mice without a horizontal axis. Consumers scroll sideways by reading one field, exactly like `wheelCells`. | full | `sparkles.input.frame` `InputFrame.wheelCellsX` (hue's per-fence code scrolling is the first consumer) |
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| INP21 | A paste must reach an application as **text, not keys** — verbatim, newlines included, so an input can insert it rather than read a newline as "submit" — while `Event` stays **pointer-free**: `std.sumtype` makes assignment `@system` for any member carrying a pointer, which would take `@safe` event handling away from every consumer (a `string` and a reference-counted `SharedBuffer` were both tried). So a paste is a run of `PasteEvent` chunks, each an inline buffer of at most `pasteChunkBytes` (40, keeping `Event` at 64 bytes) split between code points, the last one marked `last` — the native window layer's pattern (`TextCommittedEvent`'s inline buffer). A target declares whether it delivers pastes (`InputCapabilities.pasteEvents`). | full   | `events.d` `PasteEvent`, `pasteChunks`; tests `input.events.pasteChunks`, `input.events.pasteEventIsRegular`; the terminal decoders (`sparkles:tui`'s blocking reader, `ui-app`'s ring-pump assembler) turn `CSI 200~` … `CSI 201~` into chunks — `integration.pty.bracketedPasteIsOnePaste`, `ui_app.assembler.bracketedPasteIsTextNotKeys`; `terminal-view` hands a paste to its shell through libghostty's paste encoder (`terminal_view.component.pasteIsTextForTheShell`) |
