...
| ID    | Requirement                                                                                                                                                                                                                       | Status                                                                                                                                                                                  | Traces to                                        |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| MIG8  | hue's per-backend rendering must be replaced by **one widget tree per screen**. hue retains only argument parsing, document loading, the syntax pipeline, input handling and its views.                                           | partial (`9fc03551`) — markdown (all three sinks), twoslash (GUI+TUI, one window), explorer and chrome are widget trees; the raw source views still wrap `PreviewLine[]`                | [hue UI architecture](../hue/ui-architecture.md) |
| MIG9  | hue's frame-loop state must become a **single owned view-state value** with the interaction state machines this toolkit provides, replacing peer locals and mutating closures.                                                    | partial — the shared STMs (scroll/selection/disclosure/timeline/hover) replaced the ad-hoc state; the `ViewerModel` Whole for `runGui`'s remaining locals is open                       | [principles.md](./principles.md) `PRN1`, `PRN7`  |
