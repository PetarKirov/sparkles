...
| INS6 | The **sync contract**: while an inspector is open, subject position drives tree selection (pointer hover where one exists, else the host's position idiom), toggleable from the header; tree selection drives an extent highlight in the subject, scroll-following **only when the extent is fully off-screen**. | not started — first host: hue's tree-sitter inspector ([`TSI`](../hue/overlays.md))                            | host wiring over `INS1`/`INS2`; hue `foldAtCursor` idiom                   |
