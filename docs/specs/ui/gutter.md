...
| GUT2  | A cell must own its text **inline**, so rebuilding a document's chrome on every reflow allocates nothing. Building a channel must be `@nogc`.                                                                                                                                                   | shipped (`6cb7deaef`) | `SharedBuffer!(char, gutterCellInline)`; `buildingAChannelAllocatesNothing`      |
