...
| NFI3 | The **current theme** field must update when the theme is cycled live ([`THG1`](./viewer.md#live-theme-cycling-thg)); the byte/line/EGC/event stats are computed once at load. | not started | `applyTheme` (GUI); stats memoized at load                                                                                                                                                                                              |
