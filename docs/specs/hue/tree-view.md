...
| ---- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| TVU1 | **File explorer**            | filesystem tree (lazy dirs `TRV4`), file-type + git-status icons/badges (`TRV6`), open/reveal actions (`TRV7`) — the snacks-explorer use case; **entered by a directory CLI target** ([`SRC4`](./feature-requirements.md)). | partial (`d47a0d01`) — the TUI explorer ships (directory target → tree → viewer loop); git-status badges + the GUI flavor open | proposed FS adapter; `sparkles:build-primitives` walker; `SRC4` |
