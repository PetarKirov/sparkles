...
| UIA13 | **A divergence between the GUI and the TUI is a defect, not a variation.** Where an interaction tier is reachable in a backend, the observable behaviour must be identical; where it is not, the degradation is stated in the requirement itself (`precise → interactive`), never left to the implementation. Requirements are therefore scoped by tier in [viewer.md](./viewer.md), not by backend — a backend-shaped spec has nowhere to say "these must agree", which is how hue's in-document search came to be case-sensitive in the window and case-insensitive in the terminal. A backend-only file ([gui.md](./gui.md), [tui.md](./tui.md)) may hold only what the other backend structurally cannot do.                                                                                                                           | **partial** — the split has landed and six divergences are recorded in `viewer.md`; five remain unreconciled in code                                                                                                                                                                                                      | [viewer.md](./viewer.md); `sparkles.input.tier.InteractionTier`                                  |
...
| Copy button ([`COD3`](./viewer.md))   | 2      | [`WGT15`](../ui/widgets.md) + [`STM6`](../ui/state-machines.md)    | full (`8b5949a8`) — the fence header band, source-anchored ids, one feedback behavior                                   |
