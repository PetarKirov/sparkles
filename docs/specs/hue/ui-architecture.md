...
| **Window/lifecycle** |        ~21 | `InitWindow`, `SetWindowSize/Title/State`, `TakeScreenshot`, `GetScreen*`, plus the frame lifecycle (`BeginDrawing`/`EndDrawing`/`ClearBackground`/`EndScissorMode`) the painting pass left behind | a window seam in `sparkles:ui-raylib`                                            |
