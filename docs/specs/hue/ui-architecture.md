...
- **What is left is painting, not naming** — hue still fills cells by hand, and
  ...
  Seven map straight onto existing `GridCanvas` primitives (`fillRect` absorbs
  `fillRect`/`clearTo`/`fill`; `textRun` absorbs `putText`). The eighth does
  not: `tui.d:450` sets `.style.bg` on cells that already hold glyphs — a
  **tint**, where `GridCanvas.fillRect` is opaque by design (`grid_canvas`'s
  own `opaqueFillHidesWhatItCovers` test pins that). Closing it needs either a
  background-only canvas primitive or painting selection before text, as the
  GUI does. That is a decision about the canvas contract, which is why this is
  `UIA2` and not a rename.
