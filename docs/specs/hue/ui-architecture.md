...
- **The surface** — `Grid`/`Cell`/`CellStyle`/`Color` remain, and this is
  `UIA2`, not `UIA7`-style naming: **16 direct writes beside 16 `paintGrid`
  calls**. The same shape as the GUI's 13 `DrawRectangle`, and the same fix —
  chrome that should be widgets. `TerminalSession` hands the grid out
  deliberately rather than by omission, and folding it in _is_ that change.
