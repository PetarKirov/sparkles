...
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | --------------------------------------------------- |
| CLI1 | `hue [options] [target]` must render `target` — a **file path**, an **HTTP(S) URL** (`SRC3`), or a **directory** (`SRC4`); with no target it highlights hue's own bundled source.                  | partial (`74d8f6a3`) | `app.main`; `sourcePath`/`source` (URL/dir pending) |
...
| SRC4 | A **directory** target must open the **file-tree explorer** ([`tree-view.md` `TVU1`](./tree-view.md)) rooted there; selecting a file opens it in the viewer. In non-interactive modes (piped ANSI / `--html`) a directory must degrade to a reported error or a static tree listing, not a crash.                                                                       | not started       | [tree-view.md](./tree-view.md) `TVU1`; dir detection in `app.main` |
...
| HTM5 | Best-effort [`MDP`](./gui.md) parity: a markdown file may render to a **rich HTML preview** via the shared `sparkles:syntax` `MdDoc → HTML` emitter, not only highlighted source.                                                   | planned/branch-only | `DEF3` (below); the emitter exists on `feat/syntax-twoslash` — `hue --markdown` ([`TWM4`](./twoslash.md)) renders through it and the twoslash HTML `docs` use it ([`TWH4`](./twoslash.md)); syntax spec `J1`                             |
