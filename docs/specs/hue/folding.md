...
| FSR2 | Where the grammar bundle ships a **`folds.scm`** query (the tree-sitter fold convention — `@fold` captures, as nvim-treesitter uses) it must be preferred for precise, language-tuned ranges; absent it, fall back to the `FSR1` heuristic (totality).                                                                                                                                                         | full (`6b600dd2`+) — since `0a57955c` the bundle ships `folds.scm` for ada, cmake, ini, jsonnet, odin, proto, swift and zig; FSR1 still provides for every other language | `foldableSpansCst`'s query branch                                                                      |
