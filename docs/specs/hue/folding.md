...
| FLD3 | Collapsing a region must **remove its interior visual lines** from the wrapped `PreviewLine[]` ([`RND2`](./viewer.md)) and reflow/repaint; line numbers ([`NUM`](./viewer.md)) keep reflecting **physical source lines** (folded lines are skipped, not renumbered). | superseded                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `PreviewLine[]` elision; `NUM` gutter                                                       |
...
| FLD5 | **Interaction:** a gutter fold-marker click (GUI/TUI mouse) and keybindings must toggle a fold; **fold-all / unfold-all** and **fold-to-level** must be available (a documented vim-like set, e.g. `za`/`zc`/`zo`/`zR`/`zM`).                                        | partial (`361f5eb8`+) — the vim family (`za`/`zz`/`zc`/`zo`/`zR`/`zM`) + placeholder click on both backends; the full FLD5 set ships on both backends — gutter markers (▾/▸, click toggles) + `z1`–`z9` fold-to-level (vim foldlevel). Since `9f2dfc8ae` the markers are claims on the shared icon channel rather than a backend-painted column, so the arrow carries `foldHitBase + key` like the inline placeholder does and both backends toggle through one path | `ViewerModel.foldMarkers` → [`GUT6`](../ui/gutter.md) icon-slot claims; `PreviewTui.foldAt` |
...
| FSR2 | Where the grammar bundle ships a **`folds.scm`** query (the tree-sitter fold convention — `@fold` captures, as nvim-treesitter uses) it must be preferred for precise, language-tuned ranges; absent it, fall back to the `FSR1` heuristic (totality).                                                                                                                                                         | full (`6b600dd2`+) — since `bfc4eed3` the bundle ships `folds.scm` for ada, cmake, ini, jsonnet, odin, proto, swift and zig; FSR1 still provides for every other language | `foldableSpansCst`'s query branch                                                                      |
...
| FSR5 | **Diff-derived fold sources**: a diff session contributes fold ranges from its model — unchanged-region collapses ([`DVG2`](./diff-view.md)), formatting-only hunks ([`DVN2`](./diff-view.md)), resolved conflicts ([`CFV3`](./diff-view.md)), and resolved comment threads ([`DPR3`/`DCM2`](./diff-view.md)) — patch-derived folding (the diffview.nvim precedent); the HTML flavor stays pure-CSS (`FLD10`). | partial (`ddf2decb`)                                                                                                                                                      | [diff-view.md](./diff-view.md); [research: diffview.nvim](../../research/diff-review/diffview-nvim.md) |
...
| [viewer.md](./viewer.md) `RND`/`NUM`/`NAV`/`FND` | line elision, gutter, reflow, search/goto integration                                                |
