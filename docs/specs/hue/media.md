...
| IMG5 | **Image diff** — a changed image in a diff/PR session ([`DVS4`](./diff-view.md)) renders **old and new side by side** through the media-block mechanism (GUI texture · terminal graphics protocol · HTML `<img>` pair) with a dimensions/byte-size delta line; researched (WinMerge/diffy precedent), deferred. | researched/not started | `MDB*`; [diff-view.md](./diff-view.md); [research: WinMerge](../../research/diff-review/winmerge.md) |
