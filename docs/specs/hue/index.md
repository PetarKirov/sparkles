...
| [Diff & PR view](./diff-view.md) _(planned)_                    | viewing **diffs** (two files, piped unified patch, git revisions) and later **pull requests** across all four sinks: a `sparkles:diff` engine library, unified + side-by-side layouts, layered formatting-noise handling (word-level, formatting-only hunks, structural tree-sitter diff, rendered-preview diff), hunk/file navigation, PR sessions with inline comment threads                          |
