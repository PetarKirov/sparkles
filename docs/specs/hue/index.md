...
| [Gallery & multi-document nav](./gallery.md) _(planned)_       | rendering a **set** of documents: the static **HTML gallery** (index + per-file pages, prev/next header, physical-line gutter, selection domains) and interactive prev/next + index view in the GUI/TUI                                                  |
