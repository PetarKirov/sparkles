...
| FMV5  | The viewport must be preserved across enter/exit/reformat — through the shared scroll anchor ([`gui.md` `NAV5`](./gui.md)), since a reformat is a re-layout like any other. Cursor-accurate mapping via `mapCursor` over minimal edits is future work. | full (`ca124bea5`, anchored `bec7d37a`) — `mapCursor` fidelity open | dmd-fmt M5 `mapCursor`                                  |
...
| FPR12 | An applied width **parses the buffer once**: the re-highlight keeps its tree-sitter layers and the model adopts them, so the rebuild's fold scan does not re-parse the bytes just parsed. The cache retains parses under its own, much tighter cap than text+highlights, so reversing a drag over recent widths redisplays without re-parsing; an evicted parse costs one re-parse, never correctness. | full (`f03f7db75`+`303d4636b`)                                                                                             | `sparkles:syntax` `highlightParsed`               |
...
| FP7       | Parse-once apply path + the per-phase drag benchmark                                    | full (`f03f7db75`+`303d4636b`+`35a2f41fc`)                   | `FPR12`                        |
