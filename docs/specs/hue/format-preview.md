...
- **What is left on the UI thread is measured, not assumed** (`FPR12`). Applying
  a width re-highlights, re-wraps and re-lays out the buffer, and that half
  outweighed the formatting the backend had been built to move away. The rule
  that follows is to parse each formatted buffer once and let every consumer of
  that parse — highlights, folds, the inspector — share it; `format_bench.d`
  keeps the phase breakdown honest.
...
| FMV5  | The viewport must be preserved across enter/exit/reformat — through the shared scroll anchor ([`viewer.md` `NAV5`](./viewer.md)), since a reformat is a re-layout like any other. Cursor-accurate mapping via `mapCursor` over minimal edits is future work. | full (`ca124bea5`, anchored `67623407`) — `mapCursor` fidelity open | dmd-fmt M5 `mapCursor`                                  |
...
| FPR12 | An applied width **parses the buffer once**: the re-highlight keeps its tree-sitter layers and the model adopts them, so the rebuild's fold scan does not re-parse the bytes just parsed. The cache retains parses under its own, much tighter cap than text+highlights, so reversing a drag over recent widths redisplays without re-parsing; an evicted parse costs one re-parse, never correctness. | full (`2fa5bacbe`+`e1c608cef`)                                                                                             | `sparkles:syntax` `highlightParsed`               |
...
| FP7       | Parse-once apply path + the per-phase drag benchmark                                    | full (`2fa5bacbe`+`e1c608cef`+`c8826e987`)                   | `FPR12`                        |
...
| `apps/hue/src/format_bench.d`        | `@benchmark` legs for each phase of an applied width                        | `FPR9`, `FPR12`        |
