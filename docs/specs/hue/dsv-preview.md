...
> table path, layout pinned by golden glyph-grid fixtures). Everything post-CHK is
...
| DSG3 | Per-column alignment from type: numeric and date columns right-aligned, booleans centered, strings left — plus `missing` cells dimmed and `ragged` overflow tinted; the slots come from the theme like every widget ([`THM`](../ui/theme.md)).                                                                                                                                                                                    | partial (`3e18af9e`) — typed alignment ships; the `missing`/`ragged` styling slots pend                        | `buildTable` aligns from `ColumnType`           |
