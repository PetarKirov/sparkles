...
| DSK3 | A DSV document must render the **grid preview by default in every sink** — GUI, TUI, ANSI, HTML — from one shared model and one widget view (`viewDsv`), the exact shape of `MOD8`'s markdown rule.                                                                                                                                                                | full (`2ad06b91`) — golden glyph-grid fixtures pin the shared layout (`dsv_view.golden.*`); ANSI/HTML verified by smoke                                  | the ANSI/HTML `case dsv:` arms; `PreviewModel.present` routing; `apps/hue/test/fixtures/dsv/` |
