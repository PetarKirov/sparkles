...
| ID     | Section      | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CFG3` | `appearance` | `theme`, `background` mode, `crt` (monitor shader: `enabled`, `tilt`, `magnify`, `curvature`, `scanlines`, `mask`, `chromaticAberration`, `vignette`, `flicker`, `brightness`, `lensRadius`, `lensPower`, `uiReactive`, `focusHalo`, `hoverGlow`, `selectionBloom`, `dividerTension`), `pointer` (`mode`: `software` \| `system` — `PTR1`), fonts (`family`, `bold`, `italic`, `boldItalic`, `size`, `codepointMap[]`, `fontDir[]`), `window` (`width`, `height` in cells) |
...
| `CFG19` | `picker`: the default layout, the grep mode, the grep source's **per-file read cap** ([picker](./picker.md) `PKC7` — 1 MiB by default because hue re-reads per query where a cached grep would not, clamped at 10 MiB), and the **frecency store's** location. The store is machine-managed state rather than a preference (`PKR5`), and the setting exists so it can be moved off a small or network-mounted `$XDG_STATE_HOME`.                                                                                                                                   | partial     |
...
| `CFG21` | `behaviour.scrollAnchor`: what a re-layout keeps at the top of the pane ([`viewer.md` `NAV5`](./viewer.md)) — `segment` (the default: the exact wrapped segment) or `line` (the source line's first segment). It exists on the command line as `--scroll-anchor` today; this is a preference a reader sets once, which is precisely what the file is for.                                                                                                                                                                                                          | full        |
| `CFG22` | `search`: the matcher's case rule — `smartCase`, and `unicodeCaseFold` for whether the fold reaches beyond ASCII — shared by the in-document search ([viewer](./viewer.md) `FND2`) and the picker's content search ([picker](./picker.md) `PKC8`). Named after neither consumer on purpose: a `viewer.smartCase` would imply the picker's case rule is configured elsewhere, which is the category error that let hue's two in-document searches disagree in the first place. One section, one `SearchPolicy`, three call sites ([`UIA13`](./ui-architecture.md)). | full        |
...
- **The partial rows**: `CFG12` resolves the Android path
  ...
  the configured paths yet; `CFG19` wires the picker's budgets, delays and the grep
  read cap while the layout/grep presets and the frecency store await their
  own features; `CFG20` wires preview/width/formatter while the
...
- The tier-2 consumption pass landed with the pane: the lantern's
  ...
  `scroll.hScrollStep`, `search`'s case rule (`CFG22` — reaching the window's
  viewer, the workspace's, and the picker's grep source alike), the grep
  source's read cap, `timing.liveTickMs`, and the diff engine's
...
| `SET7` | Host-specific application happens through a longest-prefix apply table: theme commits re-resolve the name into the host's cycle (an unknown name stays in the config, honest and undoable, with the footer saying why nothing changed); CRT commits update running monitor shader uniforms live; pane-width commits re-arrange the dock; font commits reload in the window only (a next-launch note in v1 — the live reload needs the pt→px setup path). | full   |
