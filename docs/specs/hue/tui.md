...
| TSL3 | App-level mouse selection requires SGR mouse tracking, which **suppresses the terminal emulator's native selection** — hue must provide its own; ` ```ansi ` and table runs remain unmapped to source (`SEL6`).        | full (`8b1d2aac`)    | `TIN2`; synthetic runs lack `srcStart`        |
