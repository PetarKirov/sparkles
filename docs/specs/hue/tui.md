...
| TSL4 | ` ```ansi ` blocks should gain file-offset selection at TUI parity with the GUI's revised [`SEL6`](./gui.md) (same `PreviewLine.selSrc*`); the GUI-first **table grid selection** ([`TBL`](./gui.md)) and **copy modes** ([`SEL7`](./gui.md)/`CLI10`/`CLI11`) are a later TUI parity item. | not started          | shared `selSrc`; `table_select.d` (GUI-first) |
