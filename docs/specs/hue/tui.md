...
| TSL5 | The **code-fence** and **whole-table** copy buttons ([`COD3`](./gui.md)/[`TBL6`](./gui.md)) must render (fence: header-band right edge; table: top-border cutout) and, on click, copy the raw fence body / the table serialized as TSV via **OSC 52** (`TCL1`), with a ✔ until the state clears.                                               | full                 | `copyFenceAt` (the header-band hit) / `copyTableAt` (the `tableCopyHitBase` cutout hit) |
