...
| TSL2 | The selection must be tinted (reverse-video / SGR); a copy key (`Ctrl+C`, macOS `Cmd+C` / `super_` via CSI u / Kitty keyboard protocol, or `y`) must write `source[a..b]` to the clipboard via **OSC 52** (`TCL1`) — the terminal analog of `SEL4` — and trigger an in-app notification box near the top right.                                                                            | full (`8b1d2aac`)    | selection pass; `TCL1`; `keymap.d` (`Command.copySelection`)                                                      |
...
| TSL8 | Copying a selection must trigger a floating in-app notification box near the top right (`✓ Copied to clipboard`) timed by `Timeline` (`holdMs: 1600`), giving clear visual confirmation of the clipboard action.                                                                                                                                                                           | full                 | `tui.d` `paintToast`/`showToast`; `workspace.d` `waitDeadline`/`onWaitExpired`                                    |
