...
_**Note:** requirements shared with the GPU window moved to
[viewer.md](./viewer.md) and are listed under [Retired IDs](#retired-ids-viewer-md);
what remains here is terminal substrate — escape-sequence decoding, the
alt-screen, capability probing and OSC 52._
...
[`RND2`](./viewer.md) ("one wrapped visual-line list, painted by a single painter")
...
an off-screen VT ([`MDP12`](./viewer.md) / `sparkles:ghostty`).
...
| `FND` search & goto          | full, minus `:N`             | incremental search, `n`/`N` jumps and match tinting, all on the shared matcher and the shared input bar. A cell carries no alpha, so the window's translucent gold/orange are pre-mixed against the page background at the same 70/255 and 130/255. `:N` goto-line is the one gap (`D8`) |
...
| MDP26  | In the terminal, link text must be wrapped in **OSC 8** so the terminal itself makes it clickable. A hyperlink is opened and closed inside each redrawn run, never across the diff's cursor moves. The GUI ignores the channel — there the hand cursor is the affordance.                                                                                                                                | full                 | `TextSpan.linkId` → `Visual` → `CellT` → `Screen.render`; `MdLinkTable` |
...
## Retired IDs → `viewer.md`

Seventeen `T*` rows specified capabilities that `gui.md` already specified, which
is how the two backends were able to drift apart. They are absorbed into the
tier-scoped rows in [viewer.md](./viewer.md), which carry the old ids in their
**Supersedes** column. This table exists so a citation of a retired id still
lands somewhere useful.

| Retired  | Absorbed into                 | Note                                                                                              |
| -------- | ----------------------------- | ------------------------------------------------------------------------------------------------- |
| `TSF2`   | `NAV5`, `NAV6`, `WRP4`        | Only "SIGWINCH is the resize source" was terminal-specific; that clause stays in `TSF1`'s section |
| `TSF3`   | `RND2`                        | tui.md's own prose already said `RND2` "generalizes across backends"                              |
| `TSB1`   | `SCB1`, `SCB3`                |                                                                                                   |
| `TSB2`   | `SCB4`                        | **Divergence `D3`**: said click-to-**page**; `SCB4` says **centre**. Centre wins                  |
| `TSB3`   | `SCB2`                        | The cell degradation is now `SCB2`'s `precise → interactive` tier                                 |
| `TSL1`   | `SEL1`, `SEL2`, `SEL3`        | Cited them directly                                                                               |
| `TSL2`   | `SEL4`                        | **Divergence `D4`**: `y` copies everywhere per `TKB2`; `SEL4` had never been updated              |
| `TSL4`   | `SEL6`, `SEL7`, `TBL1`–`TBL8` | A deferral note, not a requirement                                                                |
...
| `MDP-T4` | `MDP13`–`MDP18`               | A citation list                                                                                   |
| `DIF-T3` | `SEL9`                        |                                                                                                   |
