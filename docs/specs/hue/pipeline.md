...
| PRS3 | The structural model must be able to express what its **producers** can say, not what CommonMark can write down. Two constructs already exceed it: emphasis inside preformatted text (DDoc's `$(PRE $(I Integer))`, which a fence cannot carry — `ddoc.md`'s `PRE` note), and a **sanitized HTML subset** (`<p align=…>`, `<img>`, `<a>`, `<br>`, `<sub>`/`<sup>`, `<table>`, `<details>`/`<summary>`), which GitHub renders and this repo's own `README.md` opens with. Anything the model cannot hold is lost the moment a producer serializes through a CommonMark _string_, so the model is the contract and the string is only one possible output of it. | not started | `md/model.d`; producers `DOC5`, consumers `MDP23`     |
