...
| OVL4 | The CLI must select an overlay with `--overlay <kind>[=<artifact>]` (`--list-overlays` enumerates the registered kinds); an overlay must be available across the ANSI / HTML / GUI backends per the `OVL2` contract, subject to each kind's own backend support notes.                                                                                                                                                                                                                                                                                                                                                                                                   | partial                | `app.d` `--overlay`/`--list-overlays` dispatch shipped with the one registered kind (twoslash, all backends); the registry becomes data-driven as further kinds land |
