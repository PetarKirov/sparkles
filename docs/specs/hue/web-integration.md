...
| PKG3 | The package must port Shiki's **`languageAlias`** map, cover the target sites' language set (the `ts-grammars` bundle), and fall back to plain text for unbundled grammars (the totality law). The bundle-side half of this — every label the repo writes reaching a grammar — is [`feature-requirements.md` `LNG2`](./feature-requirements.md); `PKG3` is the JS-facing alias surface on top of it. | not started | `RS1` alias map; `sparkles:syntax` grammars; `LNG2` |
