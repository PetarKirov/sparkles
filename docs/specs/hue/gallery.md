...
> [!NOTE]
> The static gallery (`GAL1`–`GAL4`, `GAL6`–`GAL9`) is shipped; the interactive
> half (`GAL5`, `GNV*`) is not started. Status
...
| ID   | Requirement                                                                                                                                                                                                                                                            | Status            | Traces to                                                                     |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ----------------------------------------------------------------------------- |
| GNV1 | The GUI and TUI must navigate the set **prev/next** (`[` / `]`, and the mouse back/forward buttons where available), reloading the newly-selected document in place — re-read, re-highlight, re-layout — without re-creating the window, font atlas, or grammar cache. | partial           | `gui.runGuiTwoslash` `loadSelected` (twoslash GUI only; `runGui`/TUI pending) |
| GNV2 | A **header bar** must show the current entry's name and summary plus its position in the set (`i/n`), in every interactive backend.                                                                                                                                    | partial           | `runGuiTwoslash` header bar (twoslash GUI only)                               |
| GNV3 | Navigation must preserve per-document view state sensibly: scroll position resets to the top of the newly-opened document, while user-level toggles (theme, line numbers, preview mode) persist across the move.                                                       | partial           | `loadSelected` resets scroll; no runtime toggles in this view yet             |
...
| G2        | Interactive: prev/next + header + index view + twoslash gutter                                          | partial           | `GNV1`–`GNV4`, `GAL5`                 |
