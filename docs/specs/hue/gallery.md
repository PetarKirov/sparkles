...
> [!NOTE]
> Shipped end to end: the static gallery (`GAL1`–`GAL4`, `GAL6`–`GAL9`) and the
...
| ID   | Requirement                                                                                                                                                                                                                                                            | Status            | Traces to                                                                                                            |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------- |
| GNV1 | The GUI and TUI must navigate the set **prev/next** (`[` / `]`, and the mouse back/forward buttons where available), reloading the newly-selected document in place — re-read, re-highlight, re-layout — without re-creating the window, font atlas, or grammar cache. | full (`e67dc1ef`) | `loadSelected` in `runGui`/`runGuiTwoslash`/`TwoslashTui`                                                            |
| GNV2 | A **header bar** must show the current entry's name and summary plus its position in the set (`i/n`), in every interactive backend.                                                                                                                                    | full (`e67dc1ef`) | header bar in `gui.d`; status bar in `twoslash_tui.d`                                                                |
| GNV3 | Navigation must preserve per-document view state sensibly: scroll position resets to the top of the newly-opened document, while user-level toggles (theme, line numbers, preview mode) persist across the move.                                                       | full (`e67dc1ef`) | `loadSelected` resets scroll/search; theme + view toggles persist                                                    |
| GNV4 | The twoslash overlay view must offer the same **physical-line gutter** as the raw/markdown view ([`NUM1`](./gui.md)), so the GUI, the TUI and the HTML gallery (`GAL4`) all number lines alike.                                                                        | full (`e6e43177`) | `runGuiTwoslash` gutter (mirrors the raw view's `NUM1` rule)                                                         |
| GNV5 | These keys and the reload path must be the primitive [navigation](./navigation.md) `LNK3` (open a linked file) and `LNK4` (back/forward history) build on — one document-open path, not two.                                                                           | full (`e67dc1ef`) | `DocLoader`/`loadSelected` — the single document-open path [navigation.md](./navigation.md) `LNK3`/`LNK4` will reuse |

## Milestones

| Milestone | Scope                                                                                                   | Status            | Requirements                          |
| --------- | ------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------- |
| G0        | Document set: directory → ordered filtered list + summaries; `--out`; reusable HTML-fragment functions  | full (`39b2d343`) | `GAL1`, `GAL8`, `SRC5`/`SRC6`, `CLI9` |
| G1        | Static HTML gallery: page shell (header/nav, single scroll, theme bg), gutter, selection domains, index | full (`a0884b5e`) | `GAL2`–`GAL4`, `GAL6`, `GAL7`, `GAL9` |
| G2        | Interactive: prev/next + header + index view + twoslash gutter                                          | full (`e67dc1ef`) | `GNV1`–`GNV4`, `GAL5`                 |
