...
- **`render-html.mjs`** (`npm run render`) — renders every fixture through
  ...
  **selection domains**. **Being retired**: its page shell is hue's own output, so
  it is moving into the app as `hue <dir> --twoslash --html --out <dir>`
  ([`gallery.md`](./gallery.md) `GAL2`), which makes the shell unit-testable and
  drops node from the preview loop. The script is deleted once the D gallery
  reproduces it.
...
| TWD3 | The **preview gallery** — VSCode-like **selection domains** (a `mousedown` handler confines a drag to the domain it starts in: code-only, or one contained annotation, layered on `TWH6`), a physical-line **gutter** (non-selectable numbers) that preserves blank lines in the copy, and the prev/next/index page shell. Prototyped in the JS harness (`render-html.mjs`); **being promoted into hue as a first-class feature** over any document set — see [`gallery.md`](./gallery.md) `GAL2`–`GAL7`, after which the JS harness is deleted. Limits: hover-popup content is not independently selectable (hover-ephemeral); `Ctrl/Cmd+A` uses the default state. | full (branch JS; `87ee7e62`, `21d722b4`, `3fa8324d`) — D port not started | [gallery.md](./gallery.md) `GAL2`–`GAL7`; superseding `libs/twoslash/examples/render-html.mjs` |
