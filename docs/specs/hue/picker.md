...
_**Status:** partial — F0 engine verified, the P0 component/scheduler seams
implemented, and the P4 grep source shipped in plain and fuzzy modes
· **Date:** 2026-09-11 · **Scope:** hue's **picker** — the
...
> hosts, `<leader>/` mounts the grep source beside it, and the preview panel
> hosts a real document pane over the same loader. Persistence, actions, and
> the later sources remain milestone work, as do grep's regex mode (`PKC16`)
> and running its scan off the UI thread (`PKC15`). Status legend and ID conventions: see the
> [overview](./index.md).
...
| `PIK10` | **A prompt edit restarts the list at its top pick.** Editing the query asks a different question, so the answer is the new first row — not whichever row the old cursor sat on. Preserving the selection is correct only while the query is _unchanged_ (a partial page growing under the cursor), which is why the reset belongs to the prompt rather than to `publish`. | full                | `PickerState.restartSelection`; `PickerHost.request`                                         |
...
| `PKS2`  | grep           | `<leader>/`  | Content search in three modes — plain, regex, fuzzy — auto-detected, falling back to fuzzy on zero hits.            | partial — plain and fuzzy scan, classified once with one visible fallback rung (`PKC9`); regex awaits `PKC16` |
...
| `PKL2`  | The **preview** must reuse `DocumentPipeline.load` and `ViewerModel` — the picker introduces no second rendering path. Reusing the pane brings its selection regime with it, which is line-wise where the main viewer is char-precise ([`D9`](./viewer.md#known-divergences)).                                                                                                                                                                       | full        | `picker_preview.d`: a `PreviewTui` over the host's own loader                                                                                                        |
...
| `PKL5`  | `<S-Tab>` must cycle the grep mode, with the active mode shown; a single-mode configuration must hide the indicator. (Today `Shift-Tab` reverses the pane focus, `PKL7`; a grep source claims it back with a context-gated row when `P4` lands.)                                                                                                                                                                                                     | full        | `GrepFinder.cycleMode`; `scanPlain`/`scanFuzzy`; the `[plain]`/`[fuzzy]` indicator; `picker_grep.scan.fuzzyAdmitsASubsequenceAndPlainDoesNot`; fff.nvim's affordance |
| `PKL6`  | A grep result must classify **definition lines** (`struct`/`fn`/`class`/`def`/`impl`), so a definition can be ranked and marked above a mention.                                                                                                                                                                                                                                                                                                     | full        | `grep_classify.classifyLine`; fff's classifier                                                                                                                       |
...
| `PKL8`  | A row wider than its panel must be **scrollable sideways**, with a real scrollbar (`SCV1`'s component, not a drawing of one) that takes pointer drags. The file-type icon and a grep row's definition marker are **pinned**: they identify the row, and scrolling them away trades the only at-a-glance marker for two more columns of path.                                                                                                         | full        | `PickerState.scroll`; `chrome.scrollbar`; `trimLeading`                                                                                                              |
| `PKL9`  | A selected row's tint must span the **whole row**, under every character it shows. A row is bounded by its own frame, not by an ancestor's clip: when the text outlives the background the last characters read as unselected, and the cursor's extent stops matching the row it marks.                                                                                                                                                              | partial     | rows column `clipX`; the reported symptom is not yet reproduced by a test                                                                                            |
| `PKL10` | The panes' scrollbars must sit **on their borders**, not inside them — the treatment a markdown fence's bars already use (`RuleEdge.centerX`/`centerY`, a sub-cell rail centred on the stroke). A bar in the padded content flow spends a row and a column that the list and the document want, in an overlay that is already the narrower half of the screen.                                                                                       | not started | `chrome.scrollbar` `edge`; the fence bars' placement                                                                                                                 |
| `PKL11` | **Grep mode title & prompt rule width**: In grep mode, the left panel displays ' Grep ' rather than ' Files '. The prompt separator rule and list bottom scrollbar track widths are bounded to the inner content columns (`panelCols - 4`), matching the panel's 2-column horizontal padding.                                                                                                                                                        | full        | `apps/hue/src/picker_view.d`                                                                                                                                         |
...
| ID      | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Status                                                                                                       | Traces to                                                                                                                                                                        |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PKC1`  | The grep source must own its scan, matching and scoring, re-entering the shared picker only with ranked results. Content lines must **never** pass through `validateCandidate`/`rank`, whose `filenameOffset == 0` invariant would award every line the filename bonus.                                                                                                                                                                                                                       | full                                                                                                         | `grepScore`; `matchText` (admission only — never `rank`); `sparkles.fuzzy.common`; `sparkles.fuzzy.rank`                                                                         |
| `PKC2`  | The corpus must be a **provider seam** — the repository walk plus the session's in-memory documents — not the walk alone. A document with no path on disk (`hue pr`, git history) must be searchable.                                                                                                                                                                                                                                                                                         | partial — the seam ships; **no host supplies a session yet**, so both pass `SessionDocs.init`                | `DocCorpus`; `SessionDocs`; `RepoDocs`; [`SRC6`](./feature-requirements.md)                                                                                                      |
| `PKC3`  | A result must be addressed as a **document handle plus line and column**, so `resolve` names a location rather than a path. Accepting a row must open at that location.                                                                                                                                                                                                                                                                                                                       | full                                                                                                         | `PickerTarget`; `ViewerModel.gotoSrcLine`                                                                                                                                        |
| `PKC4`  | The files and grep sources must share **one** repository walk. Two walks would disagree about what exists the moment the tree changed between them.                                                                                                                                                                                                                                                                                                                                           | full                                                                                                         | `globWalkGitRepository`; `PKS1`                                                                                                                                                  |
| `PKC5`  | The grep target is the document's **raw source**, not its rendered rows — the same haystack the in-document search settled on ([`FND2`](./viewer.md)).                                                                                                                                                                                                                                                                                                                                        | full                                                                                                         | `GrepDoc.source`; [viewer.md](./viewer.md)                                                                                                                                       |
| `PKC6`  | Binary content must be excluded by the `isRenderable` path filter first, then by a NUL / invalid-UTF-8 sniff **over bytes already read** — never a second read.                                                                                                                                                                                                                                                                                                                               | full                                                                                                         | `classifyContent`; [hypergrep](../../research/full-text-search/hypergrep.md)                                                                                                     |
| `PKC7`  | Files must be read whole, **capped** — 1 MiB by default, configurable to 10 MiB. The cap is lower than a cached grep's because hue re-reads per query where fff caches. Memory-mapping is refused: a file changing under a map is a fault, not an error.                                                                                                                                                                                                                                      | full                                                                                                         | `readCapped`; `picker.grepMaxFileKib`; [fff-grep](../../research/full-text-search/fff-grep.md); [ripgrep](../../research/full-text-search/ripgrep.md)                            |
| `PKC8`  | Character classes must be **ASCII by default**, with Unicode available later and switchable at runtime — not a compile-time fork of the engine.                                                                                                                                                                                                                                                                                                                                               | full                                                                                                         | `settings.searchPolicy`; `PickerHost.searchPolicy`; `SearchSettings`                                                                                                             |
| `PKC9`  | The mode must be **classified once** from the query, with exactly one visible fallback rung. A silently re-interpreted query produces a result list nobody can explain.                                                                                                                                                                                                                                                                                                                       | full                                                                                                         | `classifyQuery`; `GrepFinder.begin`/`step`; `modeLabel(mode, fellBack)`; `picker_grep.finder.aLiteralThatFindsNothingFallsBackOnceAndSaysSo`; `PKL5`                             |
| `PKC10` | A work unit is **one whole file**, and generation cancellation must be polled **every file**. fff's "refuse to abort before two matches" rule must **not** be ported — hue already publishes ranked partial pages, so the reason for it does not hold here.                                                                                                                                                                                                                                   | full                                                                                                         | `GrepScan.step`; [interactive-contracts](../../research/full-text-search/interactive-contracts.md)                                                                               |
| `PKC11` | A row must store a **512-byte window** around the match with elision, leading whitespace trimmed while the reported column stays true, plus the match ranges. 512 bytes keeps a line inside `maxDpUnits`, so scoring never silently degrades.                                                                                                                                                                                                                                                 | full                                                                                                         | `windowBytes`; `captureWindow`; `trimLeading`; the fuzzy arm asks the engine through the same window, so a row cannot claim a match the reader is never shown; fff's `GrepMatch` |
| `PKC12` | A row's identity must mix **path, line and column**. A `fingerprint()` keyed on the path alone collides across every hit in a file and freezes the list.                                                                                                                                                                                                                                                                                                                                      | full                                                                                                         | `GrepHit.fingerprint`; `sparkles.fuzzy.TopK`                                                                                                                                     |
| `PKC13` | Ranking must be **structural, not statistical**: the existing composite formula plus a definition-versus-mention term. BM25 is refused — term frequency is an anti-signal in code, IDF is distorted by generated files, and length normalisation punishes the short focused files people want.                                                                                                                                                                                                | full                                                                                                         | `grepScore`; `ScoreBreakdown.definition`; [ranked-retrieval](../../research/full-text-search/theory/ranked-retrieval.md)                                                         |
| `PKC14` | `PKL6`'s definition classifier must be a **byte heuristic during the scan**, refined by tree-sitter on the selected row only — from the parse the preview already performed. Do the expensive classification once, where it is affordable, and let the cheap one rank.                                                                                                                                                                                                                        | partial — the byte heuristic ranks and marks; the tree-sitter refinement on the selected row is unwritten    | `grep_classify.d`; [zoekt](../../research/full-text-search/trigram-indexes/zoekt.md)                                                                                             |
| `PKC15` | `poolEligible(GrepMode)` must exist from the first commit, so a mode that cannot be `@nogc` degrades **that mode** rather than the design.                                                                                                                                                                                                                                                                                                                                                    | partial — `poolEligible` exists and nothing consumes it: the scan runs on the UI thread in `PickerHost.poll` | `poolEligible`; `RawCpuPool`                                                                                                                                                     |
...
| `P4`      | The **grep** source: three modes, the definition classifier                                                                             | core shipped — `<leader>/` opens it in both hosts, classified into plain or fuzzy with one visible fallback rung, over the configured case rule and file cap; regex (`PKC16`), the raw pool (`PKC15`), session documents (`PKC2`), `PKL9`, `PKL10` and [`D9`](./viewer.md#known-divergences) remain | `PKS2`, `PKL5`, `PKL6`, `PKL8`, `PKC1`–`PKC17`       |
