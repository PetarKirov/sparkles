...
| `PKS2`  | grep           | `<leader>/`  | Content search in three modes — plain, regex, fuzzy — auto-detected, falling back to fuzzy on zero hits.            | partial — plain and fuzzy both scan (`scanText`'s `GrepMode` arms); regex awaits `PKC16`, and the mode is chosen by hand rather than classified (`PKC9`) |
