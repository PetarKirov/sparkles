...
> its mode classification (`PKC9`) and running its scan off the UI thread
> (`PKC15`). Status legend and ID conventions: see the
> [overview](./index.md).
...
| `PKS2`  | grep           | `<leader>/`  | Content search in three modes — plain, regex, fuzzy — auto-detected, falling back to fuzzy on zero hits.            | partial — plain and fuzzy ship; regex awaits `PKC16`, and the mode is chosen by hand rather than classified (`PKC9`) |
...
| ID      | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Status                                                                                                         | Traces to                                                                                                                                             |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PKC1`  | The grep source must own its scan, matching and scoring, re-entering the shared picker only with ranked results. Content lines must **never** pass through `validateCandidate`/`rank`, whose `filenameOffset == 0` invariant would award every line the filename bonus.                                                                                                                                                                                                                       | full                                                                                                           | `grepScore`; `sparkles.fuzzy.common`; `sparkles.fuzzy.rank`                                                                                           |
...
| `PKC9`  | The mode must be **classified once** from the query, with exactly one visible fallback rung. A silently re-interpreted query produces a result list nobody can explain.                                                                                                                                                                                                                                                                                                                       | not started — the mode is cycled by hand (`PKL5`); nothing classifies the query, and there is no fallback rung | `PKL5`                                                                                                                                                |
...
| `PKC11` | A row must store a **512-byte window** around the match with elision, leading whitespace trimmed while the reported column stays true, plus the match ranges. 512 bytes keeps a line inside `maxDpUnits`, so scoring never silently degrades.                                                                                                                                                                                                                                                 | full                                                                                                           | `windowBytes`; `captureWindow`; `trimLeading`; fff's `GrepMatch`                                                                                      |
