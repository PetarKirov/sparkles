...
| TBL4 | A drag that **starts** on a table cell uses the 2D grid regime (confined to that table); a text-regime drag that merely **crosses** a table extends over its whole markdown source span (so whole-document selection includes it, `SEL8`). The regime is fixed at mouse-down. | full (`c018f39a`) | `gui.d` mouse-down regime pick; `PreviewLine.selSrcStart` on table lines                           |
