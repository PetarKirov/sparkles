...
| SEL6 | A ` ```ansi ` fence body must be file-offset selectable, **block-granular**: a drag over the decoded body selects the fence body's source span `[bodyStart, bodyEnd)` (the raw ANSI incl. escapes), so it joins the same source-range selection as prose/code. (Tables use `TBL`, not this.)   | not started       | `PreviewLine.selSrcStart`/`selSrcEnd`; `codeFence`/`wrapCode` |
| SEL7 | A **copy mode** must govern how a text-regime selection is emitted: `--ansi-copy=raw\|strip` (default `raw`), toggleable at runtime with a status-bar indicator. `strip` runs `stripSgr` over `source[a..b]` (drops SGR escapes); it also applies to the ANSI code-fence copy button (`COD3`). | not started       | `CliParams.ansiCopy` (`CLI10`); `stripSgr`; copy pass         |
...
| ID   | Requirement                                                                                                                                                                                                                                             | Status      | Traces to                                                                                          |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------- |
| TBL1 | A drag inside a table must select via **smart drag**: within one cell → a **sub-cell** character range; across cells → a **rectangle** of whole cells (rows×cols spanned); **Shift** snaps the rectangle to full **rows**, **Alt** to full **columns**. | not started | `table_select.d`; `gui.d` table-regime drag                                                        |
| TBL2 | The selection must be highlighted (per selected cell's on-screen rects; a sub-cell selection clipped to its character range). Ctrl-C must copy it: a rectangle per `--table-copy=tsv\|markdown` (`CLI11`), a sub-cell → the cell substring.             | not started | `TableGridMap.cellSpans`; `table_select` serializers                                               |
| TBL3 | Screen↔cell mapping must be **wrap- and alignment-aware** — mapping a click (output line + x) to `(gridRow, gridCol, charInCell)` and back to a cell's screen rects — via a new core-cli table layout API.                                              | not started | `core_cli.ui.table.render` `TableGridMap`/`drawTableMapped`; `wrapTable`; `PreviewLine.tableIndex` |
| TBL4 | Table and text (`SEL`) selection must be **distinct regimes**; a drag stays in the regime it started in (a text-regime drag does not extend over a table, and vice-versa).                                                                              | not started | `gui.d` mouse-down regime pick                                                                     |
| TBL5 | The smart-drag→region rule and the TSV/markdown/sub-cell serialization must be **presentation-free and testable** (no raylib), consumed by the GUI painter/copy.                                                                                        | not started | `table_select.d` (pure); unit tests                                                                |
...
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `apps/hue/src/gui.d`          | `runGui`, `drawPreview`, scrollbar (`Scrollbar`/`thumbGeometry`), `offsetAt`/`srcOffsetAtCol` + `SelPos`, text/table selection regimes, copy pass (`--ansi-copy`/`--table-copy` + toggle keys), F11, env hooks, `mix`/`alpha`/`rl`                                                                                                        | `WIN*`, `FNT2/3`, `RND*`, `VIW2`, `NUM2/3/5`, `NAV*`, `SCB*`, `THG*`, `FND*`, `COD3/4`, `SEL*`, `TBL1/2/4`, `FSC*`, `DBG*` |
| `apps/hue/src/gui_preview.d`  | `PreviewModel`/`PreviewLine` (`selSrcStart`/`selSrcEnd`/`tableIndex`)/`PreviewRun`/`CodeFence`/`BandKind`, `buildPreviewModel`, `flattenPreview`/`wrapPreview`→`WrappedPreview` (+ `TableView`)/`layoutPreview`, `buildRawPlines`, `Flattener` (block/inline handlers), `hardWrapRuns`, `renderTableMapped`, `stripSgr`, `quoteBarColors` | `RND2`, `VIW1/3`, `WRP*`, `NUM1/4`, `MDP*`, `COD1/2/5`, `SEL3/6/7`, `TBL3`                                                 |
