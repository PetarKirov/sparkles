...
| TBL3 | Screen↔cell mapping must be **wrap- and alignment-aware** — mapping a click (output line + x) to `(gridRow, gridCol, charInCell)` and back to a cell's screen rects — via a new core-cli table layout API.                                              | full (`634dff4b`) | `core_cli.ui.table.render` `TableGridMap`/`drawTableMapped`; `wrapTable`; `PreviewLine.tableIndex` |
| TBL4 | Table and text (`SEL`) selection must be **distinct regimes**; a drag stays in the regime it started in (a text-regime drag does not extend over a table, and vice-versa).                                                                              | full (`debfd92d`) | `gui.d` mouse-down regime pick                                                                     |
...
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `apps/hue/src/gui.d`          | `runGui`, `drawPreview`, scrollbar (`Scrollbar`/`thumbGeometry`), `srcOffsetAtCol`/`hitAt`, text/table selection regimes, copy pass (`--ansi-copy`/`--table-copy` + toggle keys), F11, env hooks, `mix`/`alpha`/`rl`                                                                                                                      | `WIN*`, `FNT2/3`, `RND*`, `VIW2`, `NUM2/3/5`, `NAV*`, `SCB*`, `THG*`, `FND*`, `COD3/4`, `SEL*`, `TBL1/2/4`, `FSC*`, `DBG*` |
