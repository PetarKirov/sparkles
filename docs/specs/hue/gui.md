...
| TBL3 | Screen↔cell mapping must be **wrap- and alignment-aware** — mapping a click (output line + x) to `(gridRow, gridCol, charInCell)` and back to a cell's screen rects — via a new core-cli table layout API.                                                                                                                                                                                                                              | full (`634dff4b`) | `ui.components.table.render` `TableGridMap`/`drawTableMapped`; `wrapTable`; `PreviewLine.tableIndex` |
