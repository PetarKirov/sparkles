...
| `LMP10` | `<leader>vf` toggles the [format preview](./format-preview.md) and `<leader>vF` cycles its formatter; `<`/`>` nudge the ruler while the preview is active (context-gated — `[`/`]` stay set/diff navigation). | not started       | [format-preview.md](./format-preview.md)  |
