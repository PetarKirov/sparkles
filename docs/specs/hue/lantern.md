...
| `LMP7` | `<leader>f` +file/find and `<leader>/` grep are **reserved** for the [picker](./picker.md) and must not be assigned to anything else in the meantime.                               | partial — `<leader>ff` is bound (find files); `/` and the other `f` rows await their sources | [picker.md](./picker.md) `PIK`            |
