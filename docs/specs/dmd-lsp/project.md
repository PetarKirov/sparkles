...
| PRJ2  | Build settings come from **`dub describe`**, never from reading the recipe: dependency resolution, path dependencies, sub-packages, platform blocks and configuration inheritance stay dub's job. (Carve-outs, all read through sparkles:wired's SDLang reader or JSON by `sparkles.dmd_lsp.recipe`, never by line-scanning: subpackage **names** for the `PRJ7` none-target fallback — dub offers no way to list them — whose settings still come from `dub describe --root=… :name`; the configuration **names** a selection offers; and which configuration is a device build, `TGT6`.) | full (`95a85f51`) | `describeDubProject`                                                                         |
| PRJ3  | **Everything that identifies a dub build** is selectable, each defaulting to dub's own — the settings a plain `dub build` would use: configuration (`--config`), build type (`--build`), compiler (`--compiler`: its `platform=` settings and flag spelling; both spellings parse), architecture (`--arch`), dependency configurations (`--override-config`), extra version and debug identifiers (`--d-version`, `--debug`), and the `$DFLAGS` dub runs under.                                                                                                                            | full              | `DubQuery`; tests `DubQuery.argsEnvAndKey`, `DubQuery.everyFieldReachesTheDescribedSettings` |
...
| ID    | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Status            | Traces to                       |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------- |
| PRJ10 | `twoslash-extract --dub` analyzes the input in its project's context; `--dub-config`, `--dub-build`, `--dub-compiler`, `--dub-arch`, `--dub-override-config`, `--dub-d-version`, `--dub-debug` and `--dub-dflags` select the build (`PRJ3`); a device-side analysis describes the same build with the configuration swapped for the package's device one (`TGT6`). It is **off by default**, so a standalone sample is never influenced by a project that happens to contain it — the golden corpus lives inside this repo. | full (`59e623ff`) | `apps/twoslash-extract`; `EXT6` |
...
| PRJ19 | hue's `dub` settings section (`dub.config`, `dub.buildType`, `dub.compiler`, `dub.arch`, `dub.overrideConfigs`, `dub.versions`, `dub.debugIds`, `dub.dflags`) selects the build its live types describe, passed as the extractor's `--dub-*` options and read afresh for each oracle, so a settings-pane edit reaches the next file opened. | full         | `live_types.dubBuildFlags`; `DubBuildSettings`                                                                                                                  |
