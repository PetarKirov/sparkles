...
| DDC17 | Section names are matched case-insensitively (`returns:` == `Returns:`).                                                                                                                                                                                        | not started       | engine (`dmd.doc`); `29-ddoc-sections`                                                              |
| DDC18 | A line starting with `http://` or `https://` is **not** a section name, despite the colon.                                                                                                                                                                      | not started       | engine (`dmd.doc`); `29-ddoc-sections`                                                              |
...
| DDC21 | A non-standard section becomes a `### Name` heading in the body, with underscores rendered as spaces.                                                                                                                                                           | not started       | `renderDdocText` custom-section branch                                                              |
...
| DDC25 | Text in a `Params:` section before the first `name =` is dropped by the engine's row parser.                                                                                                                                                                    | not started       | engine (`ParamSection.write`); `30-ddoc-params`                                                     |
| DDC26 | A `Params:` name that matches no actual parameter still renders as a chip (documentation drift must not lose text).                                                                                                                                             | not started       | `30-ddoc-params`                                                                                    |
...
| DDC39 | Levels `##` through `######` render at the matching depth, and any number of trailing `#`s is dropped.                                                                                                                       | not started       | `H2`–`H6` macros; `33-ddoc-markdown`                                                |
...
| DDC42 | `\*` emits a literal asterisk without starting emphasis.                                                                                                                                                                     | not started       | `34-ddoc-escapes`                                                                   |
...
| DDC44 | Inline links `[text](url)` render as CommonMark links, with an optional title in single quotes, double quotes or parentheses.                                                                                                | not started       | `LINK2`/`LINK_TITLE`; `33-ddoc-markdown`                                            |
| DDC45 | Reference links work in all three shapes — `[text][ref]`, bare `[ref]`, and a `[ref]: url "title"` definition elsewhere in the same comment (`ddoc.dd:574-585`).                                                             | not started       | `33-ddoc-markdown`                                                                  |
| DDC46 | A reference label that names a **D symbol in scope** resolves to that symbol (`[Object]`).                                                                                                                                   | not started       | `SYMBOL_LINK`; `33-ddoc-markdown`                                                   |
...
| DDC48 | Bare URLs starting `http://`/`https://` are auto-detected, must contain at least one period, and are recognized before macro substitution.                                                                                   | not started       | `DDOC_LINK_AUTODETECT`; `33-ddoc-markdown`                                          |
| DDC49 | Images are links with a leading `!`, in both inline and reference form; the link text becomes alt text.                                                                                                                      | not started       | `IMAGE`/`IMAGE_TITLE`; `33-ddoc-markdown`                                           |
...
| DDC51 | Inside a `/** */` comment a `*` bullet must be doubled (the first `*` is comment margin); the same caveat applies to `+` in `/++ +/` (`ddoc.dd:692-708`).                                                                    | not started       | `33-ddoc-markdown`                                                                  |
| DDC52 | Ordered lists start with a number and a period, nest, and preserve their numbering and start index.                                                                                                                          | partial           | `OL`/`LI` render every item as `- `; `OL_START` drops the start index               |
| DDC53 | A list item may contain further block content — paragraphs, headings, code blocks, sub-items — indented to the item's content column.                                                                                        | not started       | `ddoc.dd:716-727` fixture                                                           |
| DDC54 | A table is a header row, a delimiter row and zero or more data rows separated by `\|`; leading and trailing pipes are optional, and the delimiter cell count must match the header or it is not a table (`ddoc.dd:738-746`). | partial           | the emitted table has **no delimiter row**, so the result is not a CommonMark table |
| DDC55 | Colons in the delimiter row set per-column alignment (left, right, or centered).                                                                                                                                             | partial           | `TH_ALIGN`/`TD_ALIGN` drop the alignment argument                                   |
| DDC56 | A `>`-prefixed line starts a blockquote; unprefixed lines directly following it continue it (lazy continuation), and quotes may contain headings, lists and code.                                                            | not started       | `BLOCKQUOTE`; `33-ddoc-markdown`                                                    |
| DDC57 | Three or more asterisks, underscores, or **spaced** hyphens form a horizontal rule; unspaced `---` is a code fence, not a rule.                                                                                              | not started       | `HR`; `33-ddoc-markdown`                                                            |
