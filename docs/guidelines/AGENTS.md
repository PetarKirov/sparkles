...
| `sparkles:dmd-fmt`           | `libs/dmd-fmt`            | D formatter being built on the DMD-lexer token spine per `docs/research/code-formatting/dmd-fmt-proposal.md` (M0 spikes / M1 substrate today): `spine.d` lexes with `commentToken`+`whitespaceToken`, records exact byte spans (measured via the public `Lexer.p`, clamped for multi-token scans), surfaces silently-consumed bytes (`#line`, shebang) as explicit directive entries, and proves byte-for-byte reconstruction plus token-equality against a plain lex and doc-lex offset correspondence. DMD's lexer globals are not thread-safe, so all lexing serializes on a module lock                                                                                                                                                                                                                      |
