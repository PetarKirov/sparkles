...
| `shader-compile`                | `apps/shader-compile`       | The `EFX20` pipeline: compiles the single-source D shaders (`libs/ui/shaders/effects.d` + `sparkles.ui.effect_shaders` over `sparkles:shader`) with the dcompute-enabled LDC (`$SPARKLES_SHADER_LDC`, the `sparkles/vulkan-shaders` branch) to SPIR-V, validates and optimises it, cross-compiles every `@fragment` entry point with spirv-cross to GLSL 330 and ES 100, proves each with glslang, and writes `libs/ui/shaders/generated/*.frag` — the committed artifact `sparkles.ui.effect` string-imports. `--verify` diffs and skips (exit 0) without the compiler                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- [ ] Changed a transform in `sparkles.ui.effect_shaders` or `libs/ui/shaders/effects.d`?
      Regenerate the GLSL: `SPARKLES_SHADER_LDC=<sparkles/vulkan-shaders ldc2> dub run :shader-compile`,
      and commit `libs/ui/shaders/generated/`. `--verify` tells you when it is stale.
