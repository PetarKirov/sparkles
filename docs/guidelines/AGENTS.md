...
| `sparkles:ui`               | `libs/ui`                 | Canvas-first three-level UI toolkit (state-machines / layout / widgets) for the twoslash overlay's shared visual language: a DbI `isCanvas!T` seam, a semantic `Slot`/`Palette` style layer (`defaultTwoslashPalette` — the single source the CSS/GUI/ANSI backends share), a flat-arena widget model, box-flow layout, and the `view→layout→buildDisplayList→paint` pipeline (GL-free; depends only on `sparkles:base`)                                 |
