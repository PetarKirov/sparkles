...
| `sparkles:terminal-view`     | `libs/terminal-view`      | The terminal core as an embeddable component, extracted from `apps/terminal` (spec: `docs/specs/ui-app/terminal-view.md`, `TVW`): today the key/mouse → pty encoding seam (`KeyStroke`/`encodeKeyStroke` with the byte-oracle fixtures), the OSC 10/11/12 color-query scanner, and the `@nogc` POSIX pty/process helpers; the libghostty screen, per-cell renderer and the `runApp` component follow                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `sparkles:tree-sitter`       | `libs/tree-sitter`        | D bindings for the tree-sitter C runtime: ImportC surface, RAII wrappers with `TsError` reporting, grammar dlopen (grammars supplied by the nix `ts-grammars` bundle via `$SPARKLES_TS_GRAMMAR_PATH`; all come from nixpkgs except D and SDLang, the latter maintained in-house at [`PetarKirov/tree-sitter-sdl`](https://github.com/PetarKirov/tree-sitter-sdl) and pinned in `nix/packages/tree-sitter-sdl.nix`)                                                                                                                                                                                                                                                                                                                                                                                               |
...
`sparkles:test-runner`, `sparkles:twoslash`, `sparkles:ui`, and `sparkles:versions`
are documented
...
├── dub.sdl                         # Root package; declares the 34 sub-packages
├── apps/
