...
| `shader-compile`                | `apps/shader-compile`       | The `EFX20` pipeline: compiles the single-source D shaders (`libs/ui/shaders/effects.d` + `sparkles.ui.effect_shaders` over `sparkles:shader`) with the dcompute-enabled LDC (dlang.nix's `ldc-vulkan`, the `sparkles/vulkan-shaders` branch; `nix run .#shader-compile` wraps it) to SPIR-V, validates and optimises it, cross-compiles every `@fragment` entry point with spirv-cross to GLSL 330 and ES 100, proves each with glslang, and writes `libs/ui/src/sparkles/ui/shaders/*.frag` — the committed artifact `sparkles.ui.effect` string-imports. `--verify` diffs and skips (exit 0) without the compiler                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `sparkles:shader`               | `libs/shader`               | Single-source shader vocabulary: GLSL-shaped `vec2`/`vec3`/`vec4` (native `__vector`s under LDC where the size is a power of two, a lookalike struct otherwise), the `v2`/`v3`/`v4` constructors, UFCS component reads, and the GLSL built-ins (`floor`, `mod`, `dot`, `clamp`, `mix`, …) with the specification's semantics on the CPU. Under `-d-version=SparklesShaderDevice` its `@compute`/`@fragment`/`@input`/`@uniform`/`Sampler2D` are LDC's real `ldc.dcompute` symbols; otherwise inert stand-ins, so an ordinary build sees plain D. Dependency-free                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
tree. `sparkles:nix` is specified under [`docs/specs/nix/`](../specs/nix/SPEC.md)
(SPEC + PLAN). When you add or substantially extend a library, add/extend its
docs in that location.
...
├── dub.sdl                         # Root package; declares the 50 sub-packages
├── apps/
...
│   ├── shader-compile/             # D shaders -> SPIR-V (custom LDC) -> GLSL (spirv-cross), verified
│   │   └── src/app.d               # the unit table, the pipeline, --verify (skips without the compiler)
...
│   ├── shader/src/sparkles/shader/ # attributes.d (device symbols or stand-ins), types.d (vec2/3/4, v2/v3/v4),
│   │                               # math.d (GLSL built-ins), testing.d (the tests, host-only)
│   ├── ui/shaders/                 # effects.d: the @fragment entry points (device-only, outside sourcePaths);
│   │   └── generated/              # the GLSL shader-compile derives from them — committed, string-imported
...
Bare top-level scopes (`base`, `core-cli`, `docs`, `research`, `tui`, `wired`,
`nix`, `nix-eval`, ...) are fine for genuinely cross-cutting work or early-stage
packages. `nix` doubles as the `sparkles:nix` sub-package and the flake/nixpkgs
area — use it for both. When the diff is localized to one file or subdirectory,
a dotted or slashed child scope is better.
...
- [ ] Shelling out to git? Use `runGit` (`sparkles.build_primitives.git_env`) whenever the call names its repository — `-C <dir>`, a `workDir`, or a path argument. Git exports `GIT_DIR` to every child it spawns (a `rebase` exec step, a hook, `bisect run`, a `!` alias) and it overrides all three; a `git init` fixture that inherits one has twice left `core.bare = true` in this repository's config, which kills the main checkout while every linked worktree keeps working. Programs that mean the ambient repository (`ci` under a hook, `release`) keep inheriting.
- [ ] Dependency version changes need matching `dub.selections.json` and
      `nix/dub-lock.json` updates.
- [ ] Changed a transform in `sparkles.ui.effect_shaders` or `libs/ui/shaders/effects.d`?
      Regenerate the GLSL: `nix run .#shader-compile` (Linux; it carries its own compiler),
      and commit `libs/ui/src/sparkles/ui/shaders/`. `--verify` tells you when it is stale.
      The `@compute` modules may hold no string literal (a test name is one) —
      keep their tests in a host-only module.
