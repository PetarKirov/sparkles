...
| `sparkles:core-cli`         | `libs/core-cli`           | CLI argument parsing, help formatting, UI components (table/box/header/tree/meter, live region + task list, OSC links, themes), interactive prompts, process utilities, ANSI unstyle helpers                                                                                                                                                                                                                                                                              |
...
| `sparkles:ui`               | `libs/ui`                 | Canvas-first three-level UI toolkit (state-machines / layout / widgets) for the twoslash overlay's shared visual language: a DbI `isCanvas!T` seam, a semantic `Slot`/`Palette` style layer (`defaultTwoslashPalette` — the single source the CSS/GUI/ANSI backends share), a flat-arena widget model, box-flow layout, and the `view→layout→buildDisplayList→paint` pipeline (GL-free; geometry specializes `sparkles:math`'s `Vector`, as `TermSize`/`TermPosition` do) |
...
│   ├── ui/src/sparkles/ui/          # canvas-first toolkit: geometry + style (Slot/Palette/defaultTwoslashPalette) + canvas (isCanvas!T) + widget + layout + state + display_list + interp/immediate
