...
| `hue`                       | `apps/hue`                | Interactive syntax-highlighting file viewer / live theme previewer over `sparkles:syntax` (ANSI + HTML, plus an optional raylib `--gui` backend behind the `gui` build config, on `sparkles:raylib-text`; `--gui` also renders a markdown preview and native ANSI in ` ```ansi ` fences via an off-screen `sparkles:ghostty` VT) |
...
| `sparkles:raylib-text`      | `libs/raylib-text`        | Reusable raylib text-rendering core shared by `apps/terminal` and `hue --gui`: a multi-face `FontSet` (real bold/italic variants, on-demand atlas growth, `--font-codepoint-map` routing), `drawGrapheme`/`drawSolid` + a per-run `drawText`                                                                                     |
